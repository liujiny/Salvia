 ...
 *
 * Direct port from upstream gpu_unai/gpu_inner.h.  Adaptations:
 *   - Pull in gpu_unai_compat.h for MSVC PPC compatibility shims.
 *   - All `#ifdef __arm__` paths are dead on Xbox 360 PPC; the
 *     `#else` branches always select the C generic drivers.
 *   - gpuBlending / gpuLightingTXT / gpuLightingTXTGouraud macros
 *     resolve to their Generic versions via the `#ifndef` blocks. */

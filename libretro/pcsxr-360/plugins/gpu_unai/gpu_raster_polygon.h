...
/* Xbox 360 port: direct port from upstream gpu_raster_polygon.h.
 * The PolyVertex union has BE/LE branches via `__BYTE_ORDER__` —
 * MSVC PPC defines neither, so we override the macro below to take
 * the BE branch.  Other than that, no MSVC adaptations needed.
 */

#include "gpu_unai_compat.h"

/* MSVC PPC does not define __BYTE_ORDER__/__ORDER_BIG_ENDIAN__.
 * Force the BE branch in PolyVertex below. */

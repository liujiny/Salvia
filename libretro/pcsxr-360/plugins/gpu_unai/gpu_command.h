...
/* Xbox 360 port: direct port from upstream.  `USE_GPULIB` is forced
 * on (see gpu_unai.h), so the legacy GP0 dispatcher at the bottom
 * (#ifndef USE_GPULIB block) is compiled out.  The pcsxr-360 driver
 * bridge in gpu_unai_driver.cpp provides its own GP0 dispatch loop
 * modelled on `renderer_do_cmd_list` from upstream's gpulib_if.cpp. */

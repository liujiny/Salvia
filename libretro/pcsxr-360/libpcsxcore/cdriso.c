...
/* [XBOX360] el proyecto (core.vcxproj) define USE_CHD; el cdriso.c moderno usa HAVE_CHD */
#if defined(USE_CHD) && !defined(HAVE_CHD)
#define HAVE_CHD
#endif
#ifdef HAVE_CHD
...
/* [XBOX360] __func__ es C99; el C de VS2010 usa __FUNCTION__ */
#ifndef __func__
#define __func__ __FUNCTION__
#endif
#include <sys/types.h>
/* [XBOX360] fseeko/ftello son POSIX; el XDK/MSVC solo trae fseek/ftell
 * (off_t = long 32-bit, suficiente para imagenes PSX < 2GB). */
#ifndef fseeko
#define fseeko fseek
#endif
#ifndef ftello
...
/* [XBOX360] el ppf.c del fork tiene LoadSBI() sin argumentos (lee GetIsoFile
 * internamente) y NO tiene UnloadSBI. UnloadSBI = no-op aqui. */
#define UnloadSBI() ((void)0)
...
static off_t get_size(FILE *f)
{
	off_t old, size;
	assert(f);
#if !defined(USE_LIBRETRO_VFS) && !defined(_XBOX) && !defined(_WIN32)
	struct stat st;
...
static void set_static_stdio_buffer(FILE *f)
{
	...
	/* [XBOX360] Agrandar el buffer stdio (16K por defecto es < sector) reduce el
	 * numero de lecturas fisicas en streaming secuencial. El acceso al handle lo
	 * hace SOLO el hilo lector (cdrom-async.c), asi que un buffer estatico unico
	 * es seguro. No pasarse: un setvbuf muy grande rellena todo el buffer en un
	 * fread bloqueante; 32K es un buen compromiso (refill corto). */
	...
	*tmp = 0;
	...
	while (fgets(linebuf, sizeof(linebuf), fi) != NULL) {
		...
		if (!strcmp(token, "TRACK")) {
			...
			if (sscanf(linebuf, " TRACK %02d", &t) == 1 && t != numtracks)
			...
			else if (sscanf(linebuf, " TRACK %u MODE%u/%u", &t, &mode,
					&tmpinfo[numtracks].sector_size) == 3)
				ti[numtracks].type = CDRT_DATA;
			else {
			...
			unsigned int index;
			char msf[3];
			if (sscanf(linebuf, " INDEX %02d %8s", &index, time) != 2) {
			...
			if (sscanf(linebuf, " POSTGAP %8s", time) != 1) {
	...
	for (i = 1; i <= numtracks; i++)
	{
		...
		if (i+1 <= numtracks && ti[i+1].handle == NULL) {
			...
			size_t file_size = get_size(ftmp);
			long left = file_size - ti[i].start_offset;
			if (left > 0)
				ti[i].length = left / sector_size;
			else
				ti[i].length = 0;
		}
...
static int handlechd(const char *isofile) {
	...
	/* [XBOX360] CHD_Precache no existe en Config (y precache de un CHD entero
	 * no cabe en los 512MB del 360). Deshabilitado. */
	if (0 && (chd_precache(chd_img->chd) != CHDERR_NONE))
		goto fail_io;
	...
	while (1)
	{
		struct {
			...
			uint32_t pregap;
			uint32_t postgap;
		} md = {0};   /* [XBOX360] {} vacio no es C89; {0} zero-inicializa */
		char meta[256];
...
static int opensbifile(const char *isoname) {
	...
	/* [XBOX360] LoadSBI() del fork no toma args (usa GetIsoFile) y devuelve void */
	(void)sbiname;
	LoadSBI();
	return 0;
}
...
#ifdef HAVE_CHD
/* [XBOX360] Helpers CHD con CONTEXTO explicito (struct chd_img_s *c). */

...
dbool   deh_pars = FALSE; // in wi_stuff to allow pars in modified games
...
dbool   deh_GetData(char *, char *, uint64_t *, char **, FILE *);
dbool   deh_procStringSub(char *, char *, char *, FILE *);
...
static dbool   includenotext = FALSE;
      ...
      if (!strncasecmp(inbuffer,"INCLUDE",7)) // include a file
        {
          ...
          char *nextfile;
          dbool   oldnotext = includenotext;       // killough 10/98
...
static void deh_procText(DEHFILE *fpin, FILE* fpout, char *line)
{
  ...
  static dbool   sprnames_state[NUMSPRITES+1];
  static dbool   S_sfx_state[NUMSFX];
  static dbool   S_music_state[NUMMUSIC];
...
//
dbool   deh_procStringSub(char *key, char *lookfor, char *newstring, FILE *fpout)
{
  dbool   found; // loop exit flag
  int i;  // looper
...
dbool   deh_GetData(char *s, char *k, uint64_t *l, char **strval, FILE *fpout)
{
  char *t;  // current char
  ...
  // e6y: Correction of wrong processing of Bits parameter if its value is equal to zero
  // No more desync on HACX demos.
  dbool   okrc = 1;  // assume good unless we have problems
  int i;  // iterator

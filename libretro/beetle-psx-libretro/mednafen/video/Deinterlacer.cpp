...
void Deinterlacer::InternalProcess(MDFN_Surface *surface, MDFN_Rect &DisplayRect, int32 *LineWidths, const bool field)
{
 ...
 for(int y = 0; y < DisplayRect.h / 2; y++)
 {
  ...
  if(WeaveGood)
  {
   const T* src = (const T*)(FieldBuffer->pixels + y * FieldBuffer->pitchinpix);
   T* dest = (T*)(surface->pixels + ((y * 2) + (field ^ 1) + DisplayRect.y) * surface->pitchinpix + DisplayRect.y);
  ...
  {
   const T* src = (const T*)(surface->pixels + ((y * 2) + field + DisplayRect.y) * surface->pitchinpix + DisplayRect.x);
#ifdef BOB_LORES
   T* dest = (T*)surface->pixels + (y + DisplayRect.y) * surface->pitchinpix + DisplayRect.x;
#else
  ...
  else
  {
   ...
   T* dest = (T*)surface->pixels + dly * surface->pitchinpix + DisplayRect.x;

   if(y == 0 && field)
   {
    T black = MAKECOLOR(0, 0, 0, 0);
    T* dm2 = (T*)surface->pixels + (dly - 2) * surface->pitchinpix;
  ...
  if(DeintType == DEINT_WEAVE)
  {
   const int32 *src_lw = &LineWidths[(y * 2) + field + DisplayRect.y];
   const T* src = (const T*)surface->pixels + ((y * 2) + field + DisplayRect.y) * surface->pitchinpix + DisplayRect.x;
   T* dest = (T*)FieldBuffer->pixels + y * FieldBuffer->pitchinpix;

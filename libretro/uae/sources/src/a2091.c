...
static void set_status_2 (struct wd_chip_state *wd, uae_u8 status)
{
...
static void wd_cmd_trans_addr(struct wd_chip_state *wd, struct wd_state *wds)
{
	...
	if (cyls >= tcyls)
		set_status_2(wd, CSR_BAD_STATUS);
	else
		set_status_2(wd, CSR_TRANS_ADDR);
}
...
{
	int i;
	for (i = 0; i < MAX_DUPLICATE_EXPANSION_BOARDS; i++) {
...
uae_u8 wdscsi_get (struct wd_chip_state *wd, struct wd_state *wds)
{
	...
	if (wd->sasr == WD_DATA) {
		...
		status = scsi_receive_data(wd->scsi, &v, true);
#if WD33C93_DEBUG_PIO
...
static void xt_command_done(struct wd_state *wds, uae_u8 status)
{
	...
	if (scsi->direction > 0) {
		if (scsi->cmd[0] == 0x0c) {
			...
			int i;
			xt_default_geometry(wds);
			size = wds->cdmac.xt_cyls[0] * wds->cdmac.xt_heads[0] * wds->cdmac.xt_sectors[0];
...
static void a2090_st506(struct wd_state *wd, uae_u8 b)
{
	...
	lun = cb[1] >> 5;
	unit = (lun & 1) ? 1 : 0;
	dmaaddr = (cb[6] << 16) | (cb[7] << 8) | cb[8];
	memset(cb + 12, 0, 4);
	scsi = wd->scsis[XT506_UNIT0 + unit];
	if (scsi) {
		uae_u8 cmd = cb[0];
		...
		if (cmd == 0x0c || cmd == 0xcc) {
			...
			uae_u16 cyls;
			uae_u8 precomp;
			uae_u8 reduce_write_current;
			uae_u8 secs_per_track;
			uae_u8 sdp[6];
			for (i = 0; i < sizeof sdp; i++) {
				sdp[i] = get_byte(dmaaddr + i);
			}
			user_options = sdp[0] >> 4;
			step_rate = sdp[0] & 15;
			heads = (sdp[1] >> 4) & 15;
			cyls = ((sdp[1] & 15) << 8) | sdp[2];
			precomp = sdp[3];
			reduce_write_current = sdp[4];
			secs_per_track = sdp[5];
...
static uae_u32 REGPARAM2 dmac_a2091_lget_2 (struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= 65535;
...
static uae_u32 REGPARAM2 dmac_a2091_wget_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= 65535;
	v = dmac_a2091_read_word(wd, addr);
...
static uae_u32 REGPARAM2 dmac_a2091_bget_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= 65535;
...
static void REGPARAM2 dmac_a2091_lput_3(struct wd_state *wd, uaecptr addr, uae_u32 l)
{
	addr &= 65535;
...
static void REGPARAM2 dmac_a2091_wput_3(struct wd_state *wd, uaecptr addr, uae_u32 w)
{
	addr &= 65535;
...
static void REGPARAM2 dmac_a2091_bput_3(struct wd_state *wd, uaecptr addr, uae_u32 b)
{
	b &= 0xff;
	addr &= 65535;
	if (wd->autoconfig) {
...
static uae_u32 REGPARAM2 dmac_a2091_wgeti_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v = 0xffff;
	addr &= 65535;
...
static uae_u32 REGPARAM2 dmac_a2091_lgeti_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= 65535;
	v = dmac_a2091_wgeti_2(wd, addr) << 16;
	v |= dmac_a2091_wgeti_2(wd, addr + 2);
	return v;
}

static int REGPARAM2 dmac_a2091_check_3(struct wd_state *wd, uaecptr addr, uae_u32 size)
{
	return 1;
}

static uae_u8 *REGPARAM2 dmac_a2091_xlate_2(struct wd_state *wd, uaecptr addr)
{
	addr &= 0xffff;
...
static uae_u8 *REGPARAM2 dmac_a2091_xlate (uaecptr addr)
{
	...
	if (wd)
		return dmac_a2091_xlate_2(wd, addr);
...
static int REGPARAM2 dmac_a2091_check (uaecptr addr, uae_u32 size)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_a2091_check_3(wd, addr, size);
	return 0;
}
static uae_u32 REGPARAM2 dmac_a2091_lgeti (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_a2091_lgeti_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 dmac_a2091_wgeti (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_a2091_wgeti_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 dmac_a2091_bget (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_a2091_bget_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 dmac_a2091_wget (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_a2091_wget_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 dmac_a2091_lget (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_a2091_lget_2(wd, addr);
	return 0;
}
...
static uae_u32 REGPARAM2 combitec_wget(uaecptr addr)
{
	...
	if (!wd)
		return 0;
	addr &= 65535;
	v = (wd->rom2[addr & wd->rom2_mask] << 8) | wd->rom2[(addr + 1) & wd->rom2_mask];
	return v;
}
...
static int REGPARAM2 comspec_check_3(struct wd_state *wd, uaecptr addr, uae_u32 size)
{
	return 1;
}

static uae_u8 *REGPARAM2 comspec_xlate_2(struct wd_state *wd, uaecptr addr)
{
	addr &= 0xffff;
	return wd->rom + addr;
}

static uae_u32 REGPARAM2 comspec_lget_2 (struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
...
static uae_u32 REGPARAM2 comspec_wget_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
...
static uae_u32 REGPARAM2 comspec_bget_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
...
static void REGPARAM2 comspec_lput_3(struct wd_state *wd, uaecptr addr, uae_u32 l)
{
	comspec_write_word(wd, addr + 0, l >> 16);
	comspec_write_word(wd, addr + 2, l);
}

static void REGPARAM2 comspec_wput_3(struct wd_state *wd, uaecptr addr, uae_u32 w)
{
...
static void REGPARAM2 comspec_bput_3(struct wd_state *wd, uaecptr addr, uae_u32 b)
{
	b &= 0xff;
...
static uae_u32 REGPARAM2 comspec_wgeti_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v = 0xffff;
	addr &= 65535;
	if (!(addr & 0x8000))
...
static uae_u32 REGPARAM2 comspec_lgeti_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= 65535;
	v = comspec_wgeti_2(wd, addr) << 16;
	v |= comspec_wgeti_2(wd, addr + 2);
	return v;
}
...
static uae_u8 *REGPARAM2 comspec_xlate (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return comspec_xlate_2(wd, addr);
	return default_xlate(0);
}
static int REGPARAM2 comspec_check (uaecptr addr, uae_u32 size)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return comspec_check_3(wd, addr, size);
	return 0;
}
static uae_u32 REGPARAM2 comspec_lgeti (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return comspec_lgeti_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 comspec_wgeti (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return comspec_wgeti_2(wd, addr);
	return 0;
}
...
static uae_u32 REGPARAM2 dmac_gvp_lget_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= wd->board_mask;
...
static uae_u32 REGPARAM2 dmac_gvp_wget_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= wd->board_mask;
...
static uae_u32 REGPARAM2 dmac_gvp_bget_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= wd->board_mask;
...
static void REGPARAM2 dmac_gvp_lput_3(struct wd_state *wd, uaecptr addr, uae_u32 l)
{
	addr &= wd->board_mask;
...
static void REGPARAM2 dmac_gvp_wput_3(struct wd_state *wd, uaecptr addr, uae_u32 w)
{
	addr &= wd->board_mask;
	dmac_gvp_write_word(wd, addr, w);
}
static void REGPARAM2 dmac_gvp_bput_3(struct wd_state *wd, uaecptr addr, uae_u32 b)
{
	b &= 0xff;
	addr &= wd->board_mask;
	if (wd->autoconfig) {
...
static uae_u32 REGPARAM2 dmac_gvp_wgeti_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v = 0xffff;
	addr &= wd->board_mask;
	if (addr >= GVP_ROM_OFFSET) {
...
static uae_u32 REGPARAM2 dmac_gvp_lgeti_2(struct wd_state *wd, uaecptr addr)
{
	uae_u32 v;
	addr &= wd->board_mask;
	v = dmac_gvp_wgeti_2(wd, addr) << 16;
	v |= dmac_gvp_wgeti_2(wd, addr + 2);
	return v;
}
...
static uae_u32 REGPARAM2 dmac_gvp_wget (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_gvp_wget_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 dmac_gvp_lget (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_gvp_lget_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 dmac_gvp_wgeti (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_gvp_wgeti_2(wd, addr);
	return 0;
}
static uae_u32 REGPARAM2 dmac_gvp_lgeti (uaecptr addr)
{
	struct wd_state *wd = getscsiboard(addr);
	if (wd)
		return dmac_gvp_lgeti_2(wd, addr);
	return 0;
}
...
bool a2090b_preinit (struct autoconfig_info *aci)
{
	...
	z = read_device_from_romconfig(aci->rc, ROMTYPE_A2090B, false);
	if (z) {
...
bool comspec_preinit (struct autoconfig_info *aci)
{
	...
	memset(wd->rom, 0xff, slotsize);
	wd->rom_mask = wd->rom_size - 1;
	z = read_device_from_romconfig(aci->rc, ROMTYPE_COMSPEC, false);
	if (z) {
		wd->rom_size = zfile_size32(z);
		zfile_fread (wd->rom, wd->rom_size, 1, z);
		zfile_fclose (z);
	...
	rtc = (struct rtc_msm_data*)wd->userdata;

...
#ifdef __LIBRETRO__
	cda_new (CDDA_BUFFERS, 2352, 44100);
#else
...
#ifdef __LIBRETRO__
		if (chd_hunk_info(cf, (UINT32)(dtrack->offset * CD_FRAME_SIZE / hunksize), &compr, &cbytes) == CHDERR_NONE) {
#else
...
#ifdef __LIBRETRO__
			snprintf(toc_tmp, sizeof(toc_tmp), "%s", "    ");

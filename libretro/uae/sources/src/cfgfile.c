...
static TCHAR *cfgfile_unescape_2(const TCHAR *s, const TCHAR **endpos)
{
...
static TCHAR *cfgfile_escape (const TCHAR *s, const TCHAR *escstr, bool quote, bool min)
{
	...
	if (escstr == NULL && quote)
		doquote = true;
	s2 = xmalloc (TCHAR, _tcslen (s) + cnt * 4 + 2 + 1);
	p = s2;
	if (doquote)
		*p++ = '\"';
	for (i = 0; s[i]; i++) {
...
static TCHAR *getnextentry (const TCHAR **valuep, const TCHAR separator)
{
	...
	if (value[0] == '\"') {
		s = cfgfile_unescape_2 (value, valuep);
		value = *valuep;
...
static void cfg_dowrite_7(struct zfile *f, const TCHAR *option, const TCHAR *optionext, const TCHAR *value, int d, int target, int escape)
{
	char lf = 10;
...
{
	cfg_dowrite_7(f, option, NULL, value, d, target, escape);
}
...
static void cfgfile_dwrite_bool_4(struct zfile *f, const TCHAR *option, const TCHAR *optionext, bool b)
{
	cfg_dowrite_7(f, option, optionext, b ? _T("true") : _T("false"), 1, 0, 0);
}
static void cfgfile_dwrite_bool_int(struct zfile *f, const TCHAR *option, int b)
{
	cfgfile_dwrite_bool (f, option, b != 0);
}
/*static*/ void cfgfile_write_str(struct zfile *f, const TCHAR *option, const TCHAR *value)
{
...
static void cfgfile_write_str_4(struct zfile *f, const TCHAR *option, const TCHAR *optionext, const TCHAR *value)
{
	cfg_dowrite_7(f, option, optionext, value, 0, 0, 0);
}
...
static void cfgfile_dwrite_str_4(struct zfile *f, const TCHAR *option, const TCHAR *optionext, const TCHAR *value)
{
	cfg_dowrite_7(f, option, optionext, value, 1, 0, 0);
}
void cfgfile_dwrite_strarr_5(struct zfile *f, const TCHAR *option, const TCHAR *optionext, const TCHAR *arr[], int value)
{
	const TCHAR *v;

	checkstrarr(option, arr, value);
	v = arr[value];
	cfg_dowrite(f, option, optionext, 1, 0, 0);
}
void cfgfile_dwrite_strarr(struct zfile *f, const TCHAR *option, const TCHAR *arr[], int value)
{
	cfgfile_dwrite_strarr_5(f, option, NULL, arr, value);
}
...
static void write_filesys_config (struct uae_prefs *p, struct zfile *f)
{
	...
	for (i = 0; i < p->mountitems; i++) {
		...
		if (romtype) {
			int j;
			for (j = 0; hdcontrollers[j].label; j++) {
		...
		if (ci->type == UAEDEV_DIR) {
			...
			sfilesys = cfgfile_escape_min(filesyspath);
			sgeometry = cfgfile_escape(ci->geometry, NULL, true, false);
...
static bool cfgfile_readromboard(const TCHAR *option, const TCHAR *value, struct romboard *rbp)
{
	...
	for (i = 0; i < MAX_ROM_BOARDS; i++) {
		...
		if (!_tcsicmp(option, tmp1)) {
			...
			if (s1) {
				TCHAR *p = cfgfile_unescape_2(s1, NULL);
	...
	for (i = 0; i < MAX_RAM_BOARDS; i++) {
		...
		if (!_tcsicmp(option, tmp1)) {
			...
			if (s && _tcslen(s) >= 3 * 16 - 1) {
				...
				for (k = 0; k < (int)sizeof rb->autoconfig; k++) {
					TCHAR *s2 = &s[k * 3];
					TCHAR *endptr;
					if (k + 1 < (int)sizeof rb->autoconfig && s2[2] != '.')
						break;
					s[2] = 0;
					rb->autoconfig[k] = (uae_u8)_tcstol(s2, &endptr, 16);
				}
			}
			xfree(s);
			s1 = cfgfile_option_get(value, _T("start"));
...
static void cfgfile_writeromboard(struct uae_prefs *prefs, struct zfile *f, int num, struct romboard *rb)
{
	...
	if (rb->lf.loadfile[0]) {
		...
		_tcscat(p, _T(","));
		p += _tcslen(p);
		path = cfgfile_escape(rb->lf.loadfile, NULL, true, false);
...
static int cfgfile_yesno2 (const TCHAR *option, const TCHAR *value, const TCHAR *name, int *location, bool numbercheck)
{
	if (name != NULL && _tcscmp (option, name) != 0)
		return 0;
...
static int cfgfile_yesno_int (const TCHAR *option, const TCHAR *value, const TCHAR *name, int *location)
{
	return cfgfile_yesno2 (option, value, name, location, true);
}

static int cfgfile_yesno_5 (const TCHAR *option, const TCHAR *value, const TCHAR *name, bool *location, bool numbercheck)
{
	int val;
	int ret = cfgfile_yesno2 (option, value, name, &val, numbercheck);
	if (ret == 0)
		return 0;
	if (ret < 0)
		*location = false;
...
{
	return cfgfile_yesno_5 (option, value, name, location, true);
}
...
static int cfgfile_intval_ext (const TCHAR *option, const TCHAR *value, const TCHAR *name, const TCHAR *nameext, unsigned int *location, int scale)
{
	int base = 10;
	TCHAR *endptr;
	TCHAR tmp[MAX_DPATH];
...
static int cfgfile_intval_5 (const TCHAR *option, const TCHAR *value, const TCHAR *name, unsigned int *location, int scale)
{
	return cfgfile_intval_ext (option, value, name, NULL, location, scale);
}
int cfgfile_intval (const TCHAR *option, const TCHAR *value, const TCHAR *name, int *location, int scale)
{
	unsigned int v = 0;
	int r = cfgfile_intval_ext (option, value, name, NULL, &v, scale);
	if (!r)
		return 0;
	*location = (int)v;
	return r;
}
static int cfgfile_intval_6 (const TCHAR *option, const TCHAR *value, const TCHAR *name, const TCHAR *nameext, int *location, int scale)
{
	unsigned int v = 0;
	int r = cfgfile_intval_ext (option, value, name, nameext, &v, scale);
	if (!r)
		return 0;
	*location = (int)v;
	return r;
...
static int cfgfile_path_mp (const TCHAR *option, const TCHAR *value, const TCHAR *name, TCHAR *location, int maxsz, struct multipath *mp)
{
...
static int cfgfile_rom (const TCHAR *option, const TCHAR *value, const TCHAR *name, TCHAR *location, int maxsz)
{
	...
	if (p) {
		...
		*p = 0;
		...
		rd = getromdatabycrc_2 (crc32, true);
		if (rd) {
...
static int cfgfile_parse_host (struct uae_prefs *p, TCHAR *option, TCHAR *value)
{
	...
	if (cfgfile_intval (option, value, _T("sound_frequency"), &p->sound_freq, 1)
		...
		|| cfgfile_yesno_int(option, value, _T("gfx_vrr_monitor"), &p->gfx_variable_sync)
	...
	if (cfgfile_yesno(option, value, _T("use_debugger"), &p->start_debugger)
		...
		|| cfgfile_yesno_int(option, value, _T("warp"), &p->turbo_emulation)
	...
	if (_tcscmp (option, _T("gfx_vsync")) == 0) {
		...
		return cfgfile_yesno_int (option, value, _T("gfx_vsync"), &p->gfx_apmode[APMODE_NATIVE].gfx_vsync);
	}
	if (_tcscmp (option, _T("gfx_vsync_picasso")) == 0) {
		...
		return cfgfile_yesno_int (option, value, _T("gfx_vsync_picasso"), &p->gfx_apmode[APMODE_RTG].gfx_vsync);
	}
	...
	if (_tcscmp (option, _T("displaydata")) == 0 || _tcscmp (option, _T("displaydata_pal")) == 0 || _tcscmp (option, _T("displaydata_ntsc")) == 0) {
		...
		for (i = 0; i < MAX_CHIPSET_REFRESH; i++) {
			...
			if (!cr->inuse) {
				...
				se = cfgfile_unescape_2(filter, NULL);
...
static void get_filesys_controller (const TCHAR *hdc, int *type, int *typenum, int *num)
{
	...
	if (hdcv == HD_CONTROLLER_TYPE_UAE) {
		...
		_tcscpy(control, hdc);
		extend = (TCHAR*)_tcschr(control, ',');
		if (extend)
			extend[0] = 0;
		ext = _tcsrchr(control, '_');
		if (ext) {
			int len;
			ext++;
			len = uaetcslen(ext);
			...
			for (i = 0; hdcontrollers[i].label; i++) {
				...
				if (ext2) {
					...
					if (_tcslen(ext2) == len && !_tcsnicmp(ext, ext2, len) && hdc[0] == hdcontrollers[i].label[0]) {
						if (hdcontrollers[i].romtype) {
							int j;
							for (j = 0; expansionroms[j].name; j++) {
...
bool get_hd_geometry (struct uaedev_config_info *uci)
{
	...
	if (zfile_exists (tname)) {
		...
		if (hdf_open_2 (&hfd, uci->rootdir) > 0) {
...
static bool cfgfile_read_board_rom(struct uae_prefs *p, const TCHAR *option, const TCHAR *value, struct multipath *mp)
{
	...
	for (i = 0; expansionroms[i].name; i++) {
		...
		for (j = 0; j < MAX_DUPLICATE_EXPANSION_BOARDS; j++) {
			...
			if (cfgfile_path_mp(option, value, buf, buf2, MAX_DPATH / sizeof (TCHAR), mp)) {
				if (buf2[0]) {
				...
				if (brc) {
					...
					if (ert->subtypes) {
						...
						*p = 0;
						...
						v = cfgfile_option_select(buf2, _T("subtype"), tmp);
						if (v >= 0)
					...
					if (p && _tcslen(p) >= 3 * 16 - 1) {
						int k;
						for (k = 0; k < (int)sizeof brc->roms[idx].autoconfig; k++) {
							TCHAR *s2 = &p[k * 3];
							TCHAR *endptr;
							if (k + 1 < (int)sizeof brc->roms[idx].autoconfig && s2[2] != '.')
								break;
							p[2] = 0;
							brc->roms[idx].autoconfig[k] = (uae_u8)_tcstol(s2, &endptr, 16);
						}
					}
					xfree(p);
				}
...
static int cfgfile_parse_hardware (struct uae_prefs *p, const TCHAR *option, TCHAR *value)
{
	...
	if (cfgfile_yesno(option, value, _T("immediate_blits"), &p->immediate_blits)
		...
		|| cfgfile_yesno_int(option, value, _T("genlock_aspect"), &p->genlock_aspect)
	...
	if (cfgfile_path_mp (option, value, _T("kickstart_rom_file"), p->romfile, sizeof p->romfile / sizeof (TCHAR), &p->path_rom)
		...
		|| cfgfile_path_mp(option, value, _T("picassoiv_rom_file"), p->picassoivromfile, sizeof p->picassoivromfile / sizeof(TCHAR), &p->path_rom)
	...
	if (cfgfile_yesno_int(option, value, _T("scsi_cdtv"), &tmpval)) {
		if (tmpval)
...
void cfgfile_compatibility_rtg(struct uae_prefs *p)
{
	...
	for (i = 0; i < MAX_RTG_BOARDS; i++) {
		if (!rtgs[i]) {
			...
			if (romtype) {
				int devnum;
				for (devnum = 0; devnum < MAX_DUPLICATE_EXPANSION_BOARDS; devnum++) {
					addbcromtype(p, romtype, false, NULL, devnum);
				}
			}
		}
...
static void calcformula (struct uae_prefs *prefs, TCHAR *in)
{
	...
	*p = 0;
	...
	v = calc(out, &val, tmp, sizeof(tmp) / sizeof(TCHAR));
	if (v > 0) {
		if (val - (int)val != 0.0f)
...
uae_u32 cfgfile_uaelib(TrapContext *ctx, int mode, uae_u32 name, uae_u32 dst, uae_u32 maxlen)
{
	...
	value = cfgfile_read_config_value(str);
	xfree(str);
	if (value) {
		char *s = ua(value);
...
void copy_prefs(struct uae_prefs *src, struct uae_prefs *dst)
{
	...
	for (slot = 0; slot < MAX_INPUT_SETTINGS; slot++) {

...
static void x86_ne2000(struct x86_bridge *xb)
{
	...
	if (rc) {
		...
		xb->ne2000_isa_board_state = xcalloc(struct pci_board_state, 1);
...
bool x86_bridge_init(struct autoconfig_info *aci, uae_u32 romtype, int type)
{
	...
	if (xb->type >= TYPE_2286) {
		...
		xb->cmosfile = zfile_fopen_2(path, _T("rb+"));
		if (!xb->cmosfile) {
			xb->cmosfile = zfile_fopen_2(path, _T("wb"));
		}
...
bool a2088xt_init(struct autoconfig_info *aci) { return false; }
bool a2088t_init(struct autoconfig_info *aci) { return false; }
bool a2286_init(struct autoconfig_info *aci) { return false; }
bool a2386_init(struct autoconfig_info *aci) { return false; }
bool isa_expansion_init(struct autoconfig_info *aci) { return false; }
bool x86_rt1000_init(struct autoconfig_info *aci) { return false; }

void x86_rt1000_add_unit(int ch, struct uaedev_config_info *ci, struct romconfig *rc) {}
void x86_doirq(uint8_t irqnum) {}
void x86_xt_ide_bios(struct zfile *z, struct romconfig *rc) {}
bool x86_mouse(int port, int x, int y, int z, int b) { return false; }

...
static void update_disk_statusline(int num)
{
	...
	if (!drv->diskfile)
		return;
	fname = zfile_getoriginalname(drv->diskfile);
	if (!fname)
...
static int drive_insert (drive *drv, struct uae_prefs *p, int dnum, const TCHAR *fname_in, bool fake, bool forcedwriteprotect)
{
	...
	if (drv->catweasel) {
		...
		/* PC formatted image */
		int heads, sd;
		bool can40, can80, drv525;

		drv->num_secs = 9;
		drv->ddhd = 1;
		sd = 0;

		can40 = dfxtype == DRV_525_DD || dfxtype == DRV_525_SD || dfxtype == DRV_PC_525_ONLY_40 || dfxtype == DRV_PC_525_40_80;
		can80 = dfxtype == DRV_35_DD || dfxtype == DRV_35_DD_ESCOM  || dfxtype == DRV_35_HD || dfxtype == DRV_PC_35_ONLY_80 || dfxtype == DRV_PC_525_40_80;
		drv525 = dfxtype == DRV_525_DD || dfxtype == DRV_PC_525_ONLY_40 || dfxtype == DRV_PC_525_40_80;
...
static int decode_buffer(drive *drv, uae_u16 *mbuf, int cyl, int drvsec, int ddhd, int filetype, int *drvsecp, int *sectable, int checkmode)
{
	...
	while (secwritten < drvsec) {
		...
		secheadbuf = writesecheadbuffer + trackoffs * 16;
		for (i = 0; i < 4; i++) {
...
static int drive_write_adf_amigados (drive *drv)
{
	...
	tid = &drv->trackdata[drv->cyl * 2 + side];
...
static void floppy_get_rootblock (uae_u8 *dst, int block, const TCHAR *disk_name, bool hd)
{
	...
	s = ua ((disk_name && disk_name[0] != '\0') ? disk_name : _T("empty"));
...
bool disk_creatediskfile (struct uae_prefs *p, const TCHAR *name, int type, drive_type adftype, int hd, const TCHAR *disk_name, bool ffs, bool bootable, struct zfile *copyfrom)
{
	...
	if (f && chunk) {
		...
		if (type == 0 || type == 2) {
			...
			dodos = ffs || bootable || (disk_name && disk_name[0] != '\0');
			for (i = 0; i < tracks; i++) {
				uae_u8 tmp[3 * 4];
...
void DISK_select (uae_u8 data)
{
	...
	for (dr = 0; dr < MAX_FLOPPY_DRIVES; dr++) {
		...
		selected_led = !(selected & (1 << dr)) && floppy[dr].selected_delay == 0 && dr > 0;
...
void DISK_update_predict(void)
{
	...
	for (dr = 0; dr < MAX_FLOPPY_DRIVES; dr++) {
		...
		isempty = drive_empty(drv);
		isunformatted = unformatted(drv);
		mfmpos = drv->mfmpos;
		tdsksync_on = dsksync_on;
		if (drv->tracktiming[0])
			updatetrackspeed (drv, mfmpos);
		diskevent_flag = 0;
		tword = word;
		noselected = false;
		countcycle = startcycle;
		tdsklength = dsklength;
...
void DISK_init (void)
{
	...
	for (dr = MAX_FLOPPY_DRIVES - 1; dr >= 0; dr--) {
		drive *drv = &floppy[dr];
		drv->drvnum = dr;
	...
	if (!fb || (fb && infotext)) {
		...
		if (sectable[0]) {
			if (!disk_checksum(writebuffer, NULL) &&
				...
				n = au((const char *)(writebuffer + 512 - 20 * 4 + 1));
	...
	if (idx >= 0) {
		...
		if (drv->diskfile && !drv->pcdecodedfile && (drv->filetype == ADF_EXT2 || drv->filetype == ADF_FDI || drv->filetype == ADF_IPF || drv->filetype == ADF_SCP)) {
			...
			if (z) {
				...
				secs = drive_write_pcdos(drv, z, 1);
				if (secs >= 8) {
					ok = true;
					drv->num_secs = secs;
					for (i = 0; i < drv->num_tracks; i++) {

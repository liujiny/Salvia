	...
	if(flags.Running&1)
	{
		...
		do
		{
		  ITAG inst;
			...
			{
				//ALU instruction
				// (eliminada la llamada a _Arithmetic_Debug: trabajo muerto en
				//  el bucle interno ~44100/s, no producia salida)
...
//	addr&=0x3ff;
	...
	case 0xfc:
		if(CPUSupply[addr-0xf0])
		{
			...
			// FIX (port opera opera_dsp.c:454): devolver el dato que la CPU
			// escribio en IMem, NO ruido aleatorio (era un leftover de debug).
			val=IMem[addr-0x80];
		}
		else
 ...
 void __fastcall OperandLoader(int Requests)
{
	...
	do
	{
		...
		case 3:				
				...
				{
					temp = ireadh(REGCONV[REGi][operand.r3of.R3]^RBASEx4);					
				}
				else
				{
					temp = REGCONV[REGi][operand.r3of.R3]^RBASEx4;					
				}

				OperandPool[Operands]=ireadh(temp);
				Operands++;

				if(operand.r3of.R2_DI)
				{
					temp = ireadh(REGCONV[REGi][operand.r3of.R2]^RBASEx4);				
				}
				else
				{
					temp = REGCONV[REGi][operand.r3of.R2]^RBASEx4;					
				}
				...
				{
					flags.WRITEBACK = ireadh(REGCONV[REGi][operand.r3of.R1]^RBASEx4);				 
				}
				else
				{
					flags.WRITEBACK = REGCONV[REGi][operand.r3of.R1]^RBASEx4;				 
				}

				temp = flags.WRITEBACK;
				OperandPool[Operands]=ireadh(temp);
				Operands++;
		...
		case 4:
				...
				if(operand.nrof.DI)
				{
					flags.WRITEBACK=ireadh(operand.nrof.OP_ADDR);
				}
				else
				{
					flags.WRITEBACK= operand.nrof.OP_ADDR;
				}

				temp = ireadh(flags.WRITEBACK);
				OperandPool[Operands]=temp;
		...
		case 5:
				...
				if(operand.r2of.NUMREGS)
				{
					...
					{
						flags.WRITEBACK=ireadh(REGCONV[REGi][operand.r2of.R2]^RBASEx4);					
					}
					else
					{
						flags.WRITEBACK=       REGCONV[REGi][operand.r2of.R2]^RBASEx4;
					}
				...
				{
					flags.WRITEBACK=ireadh(REGCONV[REGi][operand.r2of.R1]^RBASEx4);
				}
				else
				{
					flags.WRITEBACK=       REGCONV[REGi][operand.r2of.R1]^RBASEx4;				
				}
	...
	// if raw is zero then dont bother
	if (flags.req.raw > 0)
	{
...
int  __fastcall OperandLoaderNWB(void)
{
	...
	case 2:
	case 3:
			// we can schedule this better. OperandPool gets set on both cases so call it outside the condition

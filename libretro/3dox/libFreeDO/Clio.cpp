...
unsigned short   _clio_EIFIFO(unsigned short channel)
{
	...
	if(FIFOI[channel].StartAdr!=0)//channel enabled
	{
		...
		else
		{
			...
			if(FIFOI[channel].NextAdr!=0 && (cregs[0x304]&mask))// reload SOLO si el canal DMA sigue habilitado (opera dma_channel_enabled). Si el juego deshabilita el canal (0x308) al acabar el FMV/pista dejando NextAdr, NO recargar -> silencio en vez de bucle. mask=1<<channel.
			{
...
void   _clio_EOFIFO(unsigned short channel, unsigned short val)
{
	...
	if(FIFOO[channel].StartAdr!=0)//channel enabled
	{
		...
		else
		{
			...
			if(FIFOO[channel].NextAdr!=0 && (cregs[0x304]&mask)) //reload solo si el canal sigue habilitado (idem EI, como opera). mask=1<<(channel+16).
			{

...
__inline uint32 BitReaderBig::Read()
{
 ...
 int32 bitcnt=bitset;
 	...
 	{
 		retval=_mem_read8(buf+(point^3)^3);
 		retval>>=8-bitpoint-bitcnt;
 		retval&=mas[bitset];
 		bitpoint+=bitset;
 		return retval;
 	}
 	if(bitpoint)
 	{
 		retval=_mem_read8(buf+(point^3)^3)&mas[8-bitpoint];
		point++;
 		bitcnt-=8-bitpoint;
 	}
 	while(bitcnt>=8)
 	...
 	{
		retval<<=bitcnt;
 		retval|=_mem_read8(buf+(point^3)^3)>>(8-bitcnt);
 	}
 	bitpoint=bitcnt;

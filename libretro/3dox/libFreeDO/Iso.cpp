...
//		CDebug::DPrint(str);
	...
	case 0x8:
		...
		//
		XbusStatus|=CDST_RDY;
		/* Status IGUAL a opera cdrom_status_tagged_padded(31): 31 bytes,
		   Status[0]=opcode, Status[30]=xbus_status (donde Portfolio busca el
		   drive-status), resto 0. Con 33 bytes Portfolio lo trata como error y
		   lee drive-status=0 (no-ready) -> cuelga. Y POLST DEBE ponerse: sin el
		   no hay FIQ/acuse de ABORT (3dox no tiene tick de FIQ periodico) -> el
		   motor de musica del juego espera para siempre = bucle persistente. */
		StatusLen=31;
		Status[0]=0x8;
		for(i=1;i<31;i++)
			Status[i]=0;
		Status[30]=XbusStatus;
		Poll|=POLST;
		MEIStatus=MEI_CDROM_no_error;

		/* ABORT debe DETENER la transferencia en curso (como opera:
		   cdrom_data_clear_transfer). Si no, tras abortar el stream de audio del
		   FMV el motor CD sigue entregando sectores -> el DSP repite el ultimo
		   tramo = "ultimo segundo embuclado". Baja POLDT y vacia lo pendiente. */
		Requested = 0;
		DataLen   = 0;
		DataPtr   = 0;
		Poll     &= ~POLDT;
...
//		CDebug::DPrint(str);
	...
	case 0x0b:
		...
		//1+31+1
		XbusStatus|=CDST_RDY;
		/* Status = opera(31 bytes, drive-status en Status[30]) + POLST
		   (mismo motivo que ABORT: sin POLST no hay acuse -> bucle). */
		StatusLen=31;
		Status[0]=0xb;
		for(i=1;i<31;i++)
			Status[i]=0;
		...
		/* FLUSH vacia los buffers internos (como opera cdrom_data_clear_transfer). */
		Requested = 0;
		DataLen   = 0;
		DataPtr   = 0;
		Poll     &= ~POLDT;
	...
	case 0x10:
		...
		if((XbusStatus&CDST_TRAY)&&(XbusStatus&CDST_DISC)&&(XbusStatus&CDST_SPIN))
		{
			...
			DataPtr=0;   /* nueva lectura: el FIFO arranca al principio de Data[]
			                (opera resetea data_idx=0 siempre). Evita empezar a
			                mitad de buffer al encadenar READs = pista "entrecortada". */

...
static int mp3dec_open_file(const char *file_name, mp3dec_map_info_t *map_info)
{
    ...
    if (!file_name)
        return MP3D_E_PARAM;
    file = CreateFileA(file_name, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
...
static int mp3dec_open_file_w(const wchar_t *file_name, mp3dec_map_info_t *map_info)
{
    ...
    if (!file_name)
        return MP3D_E_PARAM;
    file = CreateFileW(file_name, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
    if (INVALID_HANDLE_VALUE == file)
        return MP3D_E_IOERROR;
    return mp3dec_open_file_h(file, map_info);
}

...
std::string& Cross::MakePathAbsolute(std::string& str)
{
    ...
    #ifdef _XBOX
    // 2. En Xbox 360, el "Directorio Actual" es siempre la ra�z del juego
    const char* xbox_prefix = "game:\\";
    str.insert(0, xbox_prefix);
    return str;
    #else
    // C�digo original para Windows PC
	wchar_t buf[512]; UINT cp; int len;
	if (!GetCurrentDirectoryW(512, buf)) return str; // A�adido check de seguridad
	...
	#else
    // C�digo para POSIX (Linux/Android/etc)

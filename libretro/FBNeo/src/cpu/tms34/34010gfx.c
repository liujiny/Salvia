...
#define LOGGFX(x)
//do { if (LOG_GFX_OPS && input_code_pressed(KEYCODE_L)) logerror x; } while (0)
...
static UINT16 dummy_shiftreg_r(UINT32 offset)
{
	return state.shiftreg[0];
}
...
static void FUNCTION_NAME(pixblt)(int src_is_linear, int dst_is_linear)
{
	...
	if (!P_FLAG)
	{
		...
		void (*word_write)(UINT32 address,UINT16 data);
		UINT16 (*word_read)(UINT32 address);
		UINT32 readwrites = 0;
		UINT32 saddr, daddr;
		XY dstxy = { 0 };
		...
		{
			word_write = TMS34010WriteWord;//TMS34010WriteWord;
			word_read = TMS34010ReadWord;//TMS34010ReadWord;
		}
...
static void FUNCTION_NAME(pixblt_r)(int src_is_linear, int dst_is_linear)
{
	...
	if (!P_FLAG)
	{
		...
		{
			word_write = TMS34010WriteWord;
			word_read = TMS34010ReadWord;
		}

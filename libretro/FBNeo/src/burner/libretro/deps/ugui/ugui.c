...
UG_S16 _UG_GetCharData(UG_CHAR encoding, const UG_U8 **p) {
    ...
    for (; t < gui->currentFont.number_of_offsets; t++) { // Seek through the offsets
        UG_U16 curr_offset = ptr_8to16(gui->currentFont.offsets + (t * 2)); // Offsets are 16-bit, split into 2 byte values

        if (gui->currentFont.range_flags[t]) { // Check range flag
            start = curr_offset; // Store range start
            range = 1; // Set flag
            ...
            skip += ((curr_offset - start) + 1); // Encoding not found in the current range, increase skip size and clear range flag
            range = 0;
...
void _UG_FontSelect(UG_FONT *font) {
    ...
    font += 2;
    gui->currentFont.number_of_range_flags = ptr_8to16(font);     // Bytes 7+8: number_of_range_flags
    font += 2;
	gui->currentFont.bytes_per_char = ptr_8to16(font);     // Bytes 9+10: Bytes per char
    font += 2;
    if (*font++) {                                         // Byte 11: 1=Width table present, 0=not present
    ...
    gui->currentFont.range_flags = font;                   // Save pointer to range flags
    font += gui->currentFont.number_of_offsets;            // Increase pointer by number of offsets

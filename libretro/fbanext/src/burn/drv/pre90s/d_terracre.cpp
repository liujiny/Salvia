...
static void DrvRenderBgLayer()
{
	...
	for (mx = 0; mx < 32; mx++) {
		for (my = 0; my < 64; my++) {
			TileIndex = (my * 32) + mx;
			Code = BURN_ENDIAN_SWAP_INT16(VideoRam[TileIndex]); //MagicSeb
...
static void DrvRenderFgLayer()
{
	...
	for (mx = 0; mx < 32; mx++) {
		for (my = 0; my < 64; my++) {
			TileIndex = (my * 32) + mx;
			Code = BURN_ENDIAN_SWAP_INT16(VideoRam[TileIndex]) & 0xff;
...
static void DrawSprites()
{
	...
	for (INT32 i = 0; i < 0x200; i += 8)	{
		INT32 Tile = BURN_ENDIAN_SWAP_INT16(pSource[1]) & 0xff; //MagicSeb
		INT32 Attrs = BURN_ENDIAN_SWAP_INT16(pSource[2]); //MagicSeb
		INT32 xFlip = Attrs & 0x04;
		INT32 yFlip = Attrs & 0x08;
		INT32 Colour = (Attrs & 0xf0) >> 4;
		INT32 sx = (BURN_ENDIAN_SWAP_INT16(pSource[3]) & 0xff) - 0x80 + 256 * (Attrs & 1); //MagicSeb
		INT32 sy = 240 - (BURN_ENDIAN_SWAP_INT16(pSource[0]) & 0xff); //MagicSeb

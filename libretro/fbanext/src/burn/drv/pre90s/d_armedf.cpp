...
static inline void DrvPaletteRecalc()
{
	...
	for (INT32 i = 0; i < 0x1000 / 2; i++) {
		INT32 d = BURN_ENDIAN_SWAP_INT16(pal[i]); //MagicSeb 
...
static inline void AssembleInputs()
{
	...
	if (scroll_type == 1) {
		...
		ptr[1] = BURN_ENDIAN_SWAP_INT16(DrvInputs[1]);
...
static void draw_layer(UINT8 *ram, UINT8 *gfxbase, INT32 scrollx, INT32 scrolly, INT32 coloff, INT32 code_and)
{
	...
	for (INT32 offs = 0; offs < 64 * 32; offs++)
	{
		...
		INT32 code = BURN_ENDIAN_SWAP_INT16(vram[offs]) & code_and; //MagicSeb
		INT32 color = BURN_ENDIAN_SWAP_INT16(vram[offs]) >> 11;//MagicSeb
...
static INT32 LegionInit()
{
	...
	if (nRet == 0) { // hack
		*((UINT16*)(Drv68KROM + 0x001d6)) = BURN_ENDIAN_SWAP_INT16(0x0001); //MagicSeb
		*((UINT16*)(Drv68KROM + 0x00488)) = BURN_ENDIAN_SWAP_INT16(0x4e71); //MagicSeb
	}

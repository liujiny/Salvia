...
static void DrvRenderSprites(INT32 CodeShift, INT32 SpriteRamSize, INT32 SpriteSize, INT32 PriDraw, INT32 xOffset, INT32 yOffset)
{
	...
	for (Offs = 0; Offs < SpriteRamSize; Offs += 4) {
		...
		sy = BURN_ENDIAN_SWAP_INT16(SpriteRam[Offs + 3 - 4]);
		if (sy == 0x2000) break;
		
		Pri = (BURN_ENDIAN_SWAP_INT16(SpriteRam[Offs + 1]) & 0x8000) >> 15;
		Colour = ((BURN_ENDIAN_SWAP_INT16(SpriteRam[Offs + 1]) & 0x3e00) >> 9) / ColourDiv;
		if(!Pri && (Colour & 0x0c) == 0x0c)	Pri = 2;
		
		if (Pri == PriDraw || PriDraw == -1) {
			xFlip = sy & 0x4000;
			sx = (BURN_ENDIAN_SWAP_INT16(SpriteRam[Offs + 1]) & 0x01ff) - 16 - 7;
			sy = (256 - 8 - SpriteSize - sy) & 0xff;
			Code = BURN_ENDIAN_SWAP_INT16(SpriteRam[Offs + 2]) >> CodeShift;

...
static void draw_background()
{
	...
	for (INT32 offs = 0; offs < 0x1000; offs++)
	{
		...
		INT32 code = (BURN_ENDIAN_SWAP_INT16(ram[ofst]) & 0x1fff) + 0x800; //MagicSeb
		INT32 color = BURN_ENDIAN_SWAP_INT16(ram[ofst+1]); //MagicSeb
...
static void draw_midground()
{
	...
	for (INT32 offs = 0; offs < 0x1000; offs++)
	{
		...
		INT32 code = (BURN_ENDIAN_SWAP_INT16(ram[ofst]) & 0x7fff) - 0x2000;
		if (code == 0x800) continue;
		INT32 color = BURN_ENDIAN_SWAP_INT16(ram[ofst+1]);
		INT32 flipy = color & 0x40;
		INT32 flipx = color & 0x20;
...
static void draw_foreground()
{
	...
	for (INT32 offs = 0; offs < 64 * 64; offs++)
	{
		...
		INT32 code = BURN_ENDIAN_SWAP_INT16(ram[ofst]) & 0x0fff;
...
static void draw_sprites()
{
	...
	while (source < finish)
	{
		...
		INT32 number = BURN_ENDIAN_SWAP_INT16(source[2]);
		INT32 xpos   = BURN_ENDIAN_SWAP_INT16(source[0])-64;
		INT32 ypos   = BURN_ENDIAN_SWAP_INT16(source[1])-16;
		INT32 flipx  = BURN_ENDIAN_SWAP_INT16(source[3]) & 0x0020;
		INT32 flipy  = BURN_ENDIAN_SWAP_INT16(source[3]) & 0x0040;
		INT32 wide  = (BURN_ENDIAN_SWAP_INT16(source[3]) & 0x0f00)>>8;
		INT32 high  = (BURN_ENDIAN_SWAP_INT16(source[3]) & 0xf000)>>12;
		INT32 colr  = (BURN_ENDIAN_SWAP_INT16(source[3]) & 0x001f);

		if ((BURN_ENDIAN_SWAP_INT16(source[3]) & 0xff00) == 0xff00) break;

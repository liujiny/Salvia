...
static inline void expand_dynamic_tiles(INT32 offset)
{
	...
	DrvGfxROM3[offset + 0] = (BURN_ENDIAN_SWAP_INT16(data) >> 12) & 0x0f;
	DrvGfxROM3[offset + 1] = (BURN_ENDIAN_SWAP_INT16(data) >>  8) & 0x0f;
	DrvGfxROM3[offset + 2] = (BURN_ENDIAN_SWAP_INT16(data) >>  4) & 0x0f;
	DrvGfxROM3[offset + 3] = (BURN_ENDIAN_SWAP_INT16(data) >>  0) & 0x0f;
...
void __fastcall f1gp_main_write_word(UINT32 address, UINT16 data)
{
	if (((address & 0xfffffe0) == 0xfff040 && nScreenStartY == 8) ||	// f1gp
		...
		DrvBgCtrl[(address >> 1) & 0x0f] = BURN_ENDIAN_SWAP_INT16(data);
		return;
	}
	...
	if ((address & 0xfc0000) == 0xc00000) {
		...
		if (BURN_ENDIAN_SWAP_INT16(p[0]) == BURN_ENDIAN_SWAP_INT16(data)) return;
...
{
	UINT16 data = BURN_ENDIAN_SWAP_INT16(*((UINT16*)(DrvRozVidRAM + offset)));

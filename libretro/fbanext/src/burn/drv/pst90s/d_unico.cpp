...
static void DrvRenderSprites(INT32 PriorityDraw)
{
	for (INT32 Offset = 0; Offset < 0x400; Offset += 4) {
		...
		INT32 sx = BURN_ENDIAN_SWAP_INT16(SpriteRam[Offset + 0]); //MagicSeb
		INT32 sy = BURN_ENDIAN_SWAP_INT16(SpriteRam[Offset + 1]); //MagicSeb
		INT32 Code = BURN_ENDIAN_SWAP_INT16(SpriteRam[Offset + 2]); //MagicSeb
		INT32 Attr = BURN_ENDIAN_SWAP_INT16(SpriteRam[Offset + 3]); //MagicSeb
...
static void Zeropnt2RenderSprites(INT32 PriorityDraw)
{
	for (INT32 Offset = 0; Offset < 0x200; Offset += 2) {
		...
		INT32 sx = BURN_ENDIAN_SWAP_INT32(SpriteRam[Offset + 0]) & 0xffff;  //MagicSeb
		INT32 sy = BURN_ENDIAN_SWAP_INT32(SpriteRam[Offset + 0]) >> 16; //MagicSeb
		INT32 Code = BURN_ENDIAN_SWAP_INT32(SpriteRam[Offset + 1]) & 0xffff; //MagicSeb
		INT32 Attr = BURN_ENDIAN_SWAP_INT32(SpriteRam[Offset + 1]) >> 16; //MagicSeb
...
static void DrvRenderLayer(INT32 Layer)
{
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < 64; mx++) {
			Attr = BURN_ENDIAN_SWAP_INT16(VideoRam[2 * TileIndex + 1]); //MagicSeb
			Code = BURN_ENDIAN_SWAP_INT16(VideoRam[2 * TileIndex + 0]);  //MagicSeb
			Colour = Attr & 0x1f;
			Flip = Attr >> 5;
			xFlip = (Flip >> 0) & 0x01;
			yFlip = (Flip >> 1) & 0x01;
...
static void Zeropnt2RenderLayer(INT32 Layer)
{
	...
	for (my = 0; my < 64; my++) {
		for (mx = 0; mx < 64; mx++) {
			Code = BURN_ENDIAN_SWAP_INT32(VideoRam[TileIndex]);  //MagicSeb
			Attr = Code >> 16;
			Code &= 0xffff;
			Colour = Attr & 0x1f;
			Flip = Attr >> 5;
...
static void DrvCalcPalette()
{
	...
	for (INT32 i = 0; i < 0x4000; i += 2) {
		 Data1 = BURN_ENDIAN_SWAP_INT16(PaletteRam[i & ~1]);  //MagicSeb
		 Data2 = BURN_ENDIAN_SWAP_INT16(PaletteRam[i |  1]);  //MagicSeb
...
static void Zeropnt2CalcPalette()
{
	...
	for (INT32 i = 0; i < 0x2000; i++) {
		 rgb0 = BURN_ENDIAN_SWAP_INT32(PaletteRam[i]); //MagicSeb

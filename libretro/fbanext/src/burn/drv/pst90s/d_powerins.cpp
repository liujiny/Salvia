...
static void TileBackground()
{
	for (INT32 offs = 256*32-1; offs >=0; offs--) {
		...
		if ( x<=-16 || x>=320 || y <=-16 || y>=224 ) continue;
			...
			INT32 attr = BURN_ENDIAN_SWAP_INT16(RamBg[offs]); //MagicSeb
...
static void TileForeground()
{
	for (INT32 offs = 0; offs < 64*32; offs++) {
		...
		if ( x<0 || x>(320-8) || y<0 || y>(224-8)) continue;
			...
			if ((BURN_ENDIAN_SWAP_INT16(RamFg[offs]) & 0x0FFF) == 0) continue; //MagicSeb
			UINT8 *d = RomFg + (BURN_ENDIAN_SWAP_INT16(RamFg[offs]) & 0x0FFF) * 32; //MagicSeb
 			UINT16 c = ((BURN_ENDIAN_SWAP_INT16(RamFg[offs]) & 0xF000) >> 8) | 0x200; //MagicSeb
...
static void DrawSprites()
{
	...
	for ( ; source < finish; source += 8 ) {
		...
		INT32	size	=	BURN_ENDIAN_SWAP_INT16(source[1]); //MagicSeb
		INT32	code	=	(BURN_ENDIAN_SWAP_INT16(source[3]) & 0x7fff) + ( (size & 0x0100) << 7 ); //MagicSeb
		INT32	sx		=	BURN_ENDIAN_SWAP_INT16(source[4]); //MagicSeb
		INT32	sy		=	BURN_ENDIAN_SWAP_INT16(source[6]); //MagicSeb
		INT32	color	=	BURN_ENDIAN_SWAP_INT16(source[7]) & 0x3F; //MagicSeb

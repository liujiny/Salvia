...
static void draw_foreground()
{
	...
	for (INT32 offs = 0; offs < 32 * 32; offs++)
	{
		...
		INT32 attr  = BURN_ENDIAN_SWAP_INT16(vram[offs]); //MagicSeb
...
static INT32 DrvDraw()
{
	if (DrvRecalc) {
		...
		for (INT32 i = 0x200 / 2; i < 0x680 / 2; i++) {
			r = (BURN_ENDIAN_SWAP_INT16(p[i]) >> 8) & 0x0f; //MagicSeb
			g = (BURN_ENDIAN_SWAP_INT16(p[i]) >> 4) & 0x0f; //MagicSeb
			b = (BURN_ENDIAN_SWAP_INT16(p[i]) >> 0) & 0x0f; //MagicSeb

...
static inline void DrvRecalcPal()
{
	...
	for (INT32 i = 0; i < 0x2000; i++) {
		if (i == 0x200) i = 0x1000;
		INT32 d = BURN_ENDIAN_SWAP_INT16(p[i]); //MagicSeb
...
static void draw_layer(INT32 scr, INT32 pri)
{
	...
	for (INT32 offs = 0; offs < 64 * 64; offs++)
	{
		...
		INT32 code = BURN_ENDIAN_SWAP_INT16(vram[ofst * 2 + 0]) & 0x3fff; //MagicSeb
		INT32 attr = BURN_ENDIAN_SWAP_INT16(vram[ofst * 2 + 1]);//MagicSeb
...
static void draw_sprites(INT32 pri)
{
	...
	{
		INT32 sx		= BURN_ENDIAN_SWAP_INT16(vram[offs + 0]) - x_offset;//MagicSeb
		INT32 sy		= BURN_ENDIAN_SWAP_INT16(vram[offs + 1]) - 0x0f;//MagicSeb
		INT32 code	= BURN_ENDIAN_SWAP_INT16(vram[offs + 2]) & 0x1fff;//MagicSeb
		INT32 attr	= BURN_ENDIAN_SWAP_INT16(vram[offs + 3]);//MagicSeb

...
// ---------------------------------------------------------------------------------------------------------------------------------------------//
	...
	{ _T("b2b")			, _T("Bang 2 Busters (Proto)")								, _T("2000"), _T("Visco"), 0x71 },		//
};
...
unsigned char* iso9660_ReadOffset(FILE* fp, long lOffset, long lSize)
{
	// check file
	if(fp == NULL) return NULL;
	
	// allocate memory
	BYTE* pszDstBuff = (BYTE*)malloc(sizeof(BYTE) * (lSize + 1));

	if(pszDstBuff == NULL) return NULL;

	// seek file offset
	fseek(fp, lOffset, SEEK_SET);

	// read segment of data
	size_t nRead = fread(pszDstBuff, 1, lSize, fp);

...
static se3208_state_t se3208_chip;
...
{
	if (address & 3)
		return se3208_read_byte(address) | se3208_read_byte(address+1)<<8 | se3208_read_byte(address+2)<<16 | se3208_read_byte(address+3)<<24;
	else
		return se3208_read_dword(address);
}

INLINE UINT16 read_word_unaligned(UINT32 address)
{
	if (address & 1)
		return se3208_read_byte(address) | se3208_read_byte(address+1)<<8;
	else
		return se3208_read_word(address);
}
...
INLINE void write_dword_unaligned(UINT32 address, UINT32 data)
{
	if (address & 3)
	{
		se3208_write_byte(address, data & 0xff);
		se3208_write_byte(address+1, (data>>8)&0xff);
		se3208_write_byte(address+2, (data>>16)&0xff);
		se3208_write_byte(address+3, (data>>24)&0xff);
	}
	else
	{
		se3208_write_dword(address, data);
	}
}
...
INLINE void write_word_unaligned(UINT32 address, UINT16 data)
{
	...
	{
		se3208_write_word(address, data);
	}
}

INLINE UINT8 SE3208_Read8(se3208_state_t *, UINT32 addr)
{
	return se3208_read_byte(addr);
}

INLINE UINT16 SE3208_Read16(se3208_state_t *, UINT32 addr)
{
	return read_word_unaligned(addr);
}

INLINE UINT32 SE3208_Read32(se3208_state_t *, UINT32 addr)
{
	return read_dword_unaligned(addr);
}

INLINE void SE3208_Write8(se3208_state_t *, UINT32 addr,UINT8 val)
{
	se3208_write_byte(addr,val);
}

INLINE void SE3208_Write16(se3208_state_t *, UINT32 addr,UINT16 val)
{
	write_word_unaligned(addr,val);
}

INLINE void SE3208_Write32(se3208_state_t *, UINT32 addr,UINT32 val)
{
...
static void BuildTable(void)
{
	int i;
	if(!OpTable)
		OpTable=(_OP *)BurnMalloc(sizeof(_OP) * 0x10000);
	for(i=0;i<0x10000;++i)
...
static void SE3208_Interrupt(se3208_state_t *se3208_state)
{
	...
	if(!(TESTFLAG(FLAG_AUT)))
		...
		se3208_state->PC=SE3208_Read32(se3208_state, 4*se3208_state->irq_callback(0));
}
...
INT32 se3208Run(INT32 cycles)
{
	...
	do
	{
		UINT16 Opcode=se3208_fetch_word(se3208_state->PC);
	...
	se3208_state->total_cycles += cycles - se3208_state->icount;
...
{
	se3208_state_t *se3208_state = &se3208_chip;
	return se3208_state->total_cycles;
}
...
UINT32 se3208GetPC(INT32 /*cpu*/)
{
	...
	return se3208_state->PC;
}

void se3208RunEnd()
{
	se3208_state_t *se3208_state = &se3208_chip;
	se3208_state->icount = 0;
}
...
void se3208_set_irq_line(int line,int state)
{
	...
	if(line==0x20)//INPUT_LINE_NMI)	//NMI

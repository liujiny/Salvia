	...
	if (currprefs.scsi /*&& currprefs.win32_automount_cddrives*/) {
		...
		for (i = 0; i < 32; i++) {
			if (mask & (1 << i)) {
				...
				ci.surfaces = 1;
				ci.blocksize = 2048;
				idx = set_filesys_unit_1 (i + cd_unit_offset, &ci);
				allocuci (&currprefs, nr, idx);
				nr++;
			}
		}
...
static void do_info (Unit *unit, dpacket packet, uaecptr info, bool disk_info)
{
	...
	if (unit->ui.unknown_media) {
		...
		put_long (info + 20, blocksize); /* bytesperblock */
		put_long (info + 24, dostype == DISK_TYPE_DOS && kickstart_version >= 36 ? DISK_TYPE_DOS_FFS : dostype); /* disk type */
...
#else
		find_aino (unit, 0, n->fullname, &err);
#endif
		if (err == 0)
			notify_send (unit, n);
	}
...
static uae_u32 REGPARAM2 fsmisc_helper (TrapContext *context)
{
	...
	case 3:
		t = getlocaltime ();
		secs = (uae_u32)t - (8 * 365 + 2) * 24 * 60 * 60;
		return secs;
	}
	return 0;
}
	...
	action_read (Unit *unit, dpacket packet)
{
	...
	if (size == 0) {
		...
		actual = fs_read (k->fd, buf, size);
			...
			/* Upstream: el for venia sin clausula de inicio y con i sin
			 * inicializar, o sea que el bucle arrancaba en un valor cualquiera. */
			for (i = 0; i < actual; i++)
...
static uae_u32 REGPARAM2 filesys_dev_bootfilesys (TrapContext *context)
{
	...
	if (get_long (parmpacket + PP_FSPTR) && !get_long (parmpacket + PP_ADDTOFSRES)) {
		...
		pf = get_long (parmpacket + PP_FSHDSTART + 8); // fse_PatchFlags
		for (i = 0; i < 32; i++) {
			if (pf & (1 << i))
...
static int legalrdbblock (UnitInfo *uip, int block)
{
	if (block <= 0)
		return 0;
	if ((uae_u64)block >= (uip->hf.virtsize / uip->hf.ci.blocksize) )
		return 0;
	return 1;
}
...
static void dump_rdb (UnitInfo *uip, struct hardfiledata *hfd, uae_u8 *bufrdb, uae_u8 *buf, int readblocksize)
{
	...
	for (i = 0; i < 100; i++) {
		...
		dostype = rl (buf + 32);
		version = (buf[36] << 8) | buf[37];
		revision = (buf[38] << 8) | buf[39];
...
static int rdb_mount (UnitInfo *uip, int unit_no, int partnum, uaecptr parmpacket)
{
	...
	if ((lastblock > 0) && (((uae_u64)lastblock * hfd->ci.blocksize) > hfd->virtsize) ) {
		rdbmnt

...
static int cfgfile_parse_host (struct uae_prefs *p, TCHAR *option, TCHAR *value)
{
	...
	for (i = 0; i < MAX_SPARE_DRIVES; i++) {
		...
		if (cfgfile_path_mp (option, value, tmpbuf, p->dfxlist[i], sizeof p->dfxlist[i] / sizeof (TCHAR), &p->path_floppy)) {
#if 0
			if (i < 4 && !p->df[i][0])
...
cfgfile_path (option, value, _T("floppy0soundext"), p->floppyslots[0].dfxclickexternal, sizeof p->floppyslots[0].dfxclickexternal / sizeof (TCHAR))
	...
	if (_tcscmp (option, _T("kbd_lang")) == 0) {
		...
		if ((l = KBD_LANG_DE, strcasecmp (value, _T("de")) == 0)
			...
			|| (l = KBD_LANG_TR, strcasecmp (value, _T("tr")) == 0))
			p->keyboard_lang = l;
		else
...
static int cfgfile_parse_hardware (struct uae_prefs *p, const TCHAR *option, TCHAR *value)
{
	...
	if (cfgfile_path_mp (option, value, _T("kickstart_rom_file"), p->romfile, sizeof p->romfile / sizeof (TCHAR), &p->path_rom)
		...
		|| cfgfile_path_mp (option, value, _T("rtc_file"), p->rtcfile, sizeof p->rtcfile / sizeof (TCHAR), &p->path_rom)
...
int cfgfile_load (struct uae_prefs *p, const TCHAR *filename, int *type, int ignorelink, int userconfig)
{
	...
	memset(tmp, 0, MAX_DPATH * sizeof(TCHAR));
...
int parse_cmdline_option (struct uae_prefs *p, TCHAR c, const TCHAR *arg)
{
	...
	case 'l':
		...
		else if (0 == strcasecmp(arg, _T("tr")))
...
uae_u32 cfgfile_modify (uae_u32 index, TCHAR *parms, uae_u32 size, TCHAR *out, uae_u32 outsize)
{
	...
	if (index != 0xffffffff) {
		...
		if (configsearch) {
			...
			size_t inlen = _tcslen (configsearch);
			int joker = 0;

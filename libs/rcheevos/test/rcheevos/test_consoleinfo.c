...
static void test_memory(uint32_t console_id, uint32_t expected_total_memory, uint32_t expected_searchable_memory)
{
  const rc_memory_regions_t* regions = rc_console_memory_regions(console_id);
  uint32_t total_memory = 0;
  uint32_t searchable_memory = 0;
  ...
  for (i = 0; i < regions->num_regions; ++i) {
    uint32_t region_size = (regions->region[i].end_address - regions->region[i].start_address + 1);
    total_memory += region_size;

    if (regions->region[i].type != RC_MEMORY_TYPE_UNUSED)
      searchable_memory += region_size;
  ...
  ASSERT_NUM_EQUALS(searchable_memory, expected_searchable_memory);
...
void test_consoleinfo(void) {
  ...
  /* memory maps */
  TEST_PARAMS3(test_memory, RC_CONSOLE_UNKNOWN, 0x000000, 0x000000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_3DO, 0x200000, 0x200000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_AMIGA, 0x100000, 0x100000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_AMSTRAD_PC, 0x090000, 0x090000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_APPLE_II, 0x020000, 0x020000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ARCADE, 0x000000, 0x000000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ARCADIA_2001, 0x000300, 0x000300);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ARDUBOY, 0x000F00, 0x000F00);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ATARI_2600, 0x000080, 0x000080);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ATARI_7800, 0x010000, 0x010000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ATARI_JAGUAR, 0x200000, 0x200000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ATARI_JAGUAR_CD, 0x200000, 0x200000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ATARI_LYNX, 0x010000, 0x010000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_COLECOVISION, 0x008400, 0x008400);
  TEST_PARAMS3(test_memory, RC_CONSOLE_COMMODORE_64, 0x010000, 0x010000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_DREAMCAST, 0x01000000, 0x01000000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ELEKTOR_TV_GAMES_COMPUTER, 0x001800, 0x001700);
  TEST_PARAMS3(test_memory, RC_CONSOLE_FAIRCHILD_CHANNEL_F, 0x010C40, 0x010C40);
  TEST_PARAMS3(test_memory, RC_CONSOLE_FAMICOM_DISK_SYSTEM, 0x010000, 0x010000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_GAMEBOY, 0x034000, 0x02DFA0);
  TEST_PARAMS3(test_memory, RC_CONSOLE_GAMEBOY_COLOR, 0x034000, 0x033FA0);
  TEST_PARAMS3(test_memory, RC_CONSOLE_GAMEBOY_ADVANCE, 0x058000, 0x058000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_GAMECUBE, 0x01800000, 0x01800000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_GAME_GEAR, 0x00A000, 0x00A000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_INTELLIVISION, 0x040080, 0x03BC00);
  TEST_PARAMS3(test_memory, RC_CONSOLE_INTERTON_VC_4000, 0x000600, 0x000600);
  TEST_PARAMS3(test_memory, RC_CONSOLE_MAGNAVOX_ODYSSEY2, 0x000140, 0x000140);
  TEST_PARAMS3(test_memory, RC_CONSOLE_MASTER_SYSTEM, 0x00A000, 0x00A000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_MEGA_DRIVE, 0x020000, 0x020000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_MEGADUCK, 0x010000, 0x00FFA0);
  TEST_PARAMS3(test_memory, RC_CONSOLE_MSX, 0x080000, 0x080000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_MS_DOS, 0x4200000, 0x4140000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_NEOGEO_POCKET, 0x004000, 0x004000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_NEO_GEO_CD, 0x010000, 0x010000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_NINTENDO, 0x010000, 0x010000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_NINTENDO_64, 0x800000, 0x800000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_NINTENDO_DS, 0x01004000, 0x00404000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_NINTENDO_DSI, 0x01004000, 0x01004000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_ORIC, 0x010000, 0x010000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_PC8800, 0x011000, 0x011000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_PC_ENGINE, 0x02000, 0x02000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_PC_ENGINE_CD, 0x42800, 0x42800);
  ...
  TEST_PARAMS3(test_memory, RC_CONSOLE_SEGA_CD, 0x0B0000, 0x0B0000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_SG1000, 0x006000, 0x006000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_SUPER_CASSETTEVISION, 0x010000, 0x00B000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_SUPER_NINTENDO, 0x0A0800, 0x0A0800);
  TEST_PARAMS3(test_memory, RC_CONSOLE_SUPERVISION, 0x006000, 0x006000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_THOMSONTO8, 0x080000, 0x080000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_TI83, 0x08000, 0x08000);
  TEST_PARAMS3(test_memory, RC_CONSOLE_TIC80, 0x018000, 0x018000);

...
inline void DrawRectAlpha(SDL_Surface* dest, SDL_Rect rect, SDL_Color color, Uint8 alpha) {
    // 1. Crear una superficie temporal del tama�o del rect�ngulo
    ...
    // 4. Hacer el Blit a la superficie de destino (aqu� ocurre la mezcla)
...
/*inline void DrawRectAlpha(SDL_Surface* dest, SDL_Rect rect, SDL_Color color, Uint8 alpha) {
    ...
    // 1. Crear el p�xel base una sola vez
    if (alphaPixel == NULL) {
        alphaPixel = SDL_CreateRGBSurface(SDL_SWSURFACE, 1, 1, 32, 
                                          0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
    }

    // 2. Cambiar el color y el alpha del p�xel (Operaciones instant�neas)
    ...
    // 3. Estirar el p�xel al tama�o del rect�ngulo (Stretch Blit)
    // SDL_SoftStretch es mucho m�s r�pido que crear una superficie entera

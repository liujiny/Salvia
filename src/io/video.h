...
inline void check_center(uint16_t* src, uint16_t*& dst, int sw, int sh, std::size_t spitch, 
	...
	// 1. Configuraci�n de dimensiones
    const int out_w = sw * scale;
    const int out_h = sh * scale;
	src_stride = 0;
	dst_stride = 0;

	// 2. Comprobaci�n de l�mites (Safety Check)
    // Si el resultado 3x es m�s grande que la resoluci�n de pantalla actual, abortamos
    if (out_w > dw || out_h > dh) {
        // Opcional: Podr�as hacer un fallback a un Blit 1:1 aqu�
        return; 
    }
...
inline void fast_video_blit_xbox(const t_scale_props& props) {
    ...
    for (int y = 0; y < props.sh; y++) {
        // XMemCpy es una funci�n intr�nseca del XDK que usa registros de 128 bits (VMX)
        // Es ideal para copiar l�neas de p�xeles r�pidamente.
        XMemCpy(d_ptr, s_ptr, bytes_per_line);

        // Avanzamos los punteros sumando el stride (salto de l�nea)
        s_ptr += src_stride_px;
        d_ptr += dst_stride_px;
    }
}
...
inline void calcDestDimFromRatio(int sw, int sh, int dw, int dh, float ratio, int& out_w, int& out_h, int& inv_scale_x_fp, int& inv_scale_y_fp) {
    ...
    // Usamos float para el c�lculo intermedio y convertimos a punto fijo 16.16
...
inline void scale_software_fixed_point_safe2(const t_scale_props& props) {
    ...
    // Restamos un peque�o margen para asegurar que el acumulador no desborde sw-1
    ...
    for (int y = 0; y < out_h; y++) {
        ...
        // BUCLE MAESTRO: Volvemos al acumulador simple (lo m�s r�pido)
...
inline void scale_software_fixed_point_xbox_final(const t_scale_props& props) {
    ...
    for (int y = 0; y < out_h; y++) {
		...
		// Sugerencia para la cach� de la Xbox 360
		__dcbt(0, line_src); 
		...
		for (; x <= safe_out_w - 8; x += 8) {
			// Pre-calculamos los �ndices para romper la dependencia de la suma
			int i0 = curr_x_fp >> 16;
...
inline void fast_video_blit(const t_scale_props& props) {
    ...
    // 1. Usar check_center (escala 1 ya que no hay escalado manual aqu�)
    // Esto ajustar� el puntero 'dst' al punto exacto de centrado.
    check_center(props.src, dst_ptr, props.sw, props.sh, props.spitch, props.dw, props.dh, props.dpitch, 1, src_stride, dst_stride);

    // 2. El ancho a copiar en bytes (cada p�xel uint16_t son 2 bytes)
    const std::size_t bytes_per_line = props.sw * sizeof(uint16_t);

    // 3. Bucle de copiado
    for (int y = 0; y < props.sh; y++) {
        // Usamos memcpy para m�xima velocidad por l�nea
        // Destino: puntero centrado + salto de l�nea (en p�xeles)
        // Origen: puntero base + salto de l�nea (en p�xeles)
...
inline void scale2x_software(const t_scale_props& props) {
	...
	// 1. Usar check_center (escala 2 ya que no hay escalado manual aqu�)
    // Esto ajustar� el puntero 'dst' al punto exacto de centrado.
	...
	for (int y = 0; y < props.sh; y++) {
        ...
        for (int x = 0; x < props.sw; x++) {
            ...
            // Duplicamos el p�xel horizontalmente
            line_dst1[x * 2] = pixel;
            line_dst1[x * 2 + 1] = pixel;
            // Duplicamos la l�nea completa verticalmente
            line_dst2[x * 2] = pixel;
...
inline void scale3x_software(const t_scale_props& props) {
    ...
    for (int y = 0; y < props.sh; y++) {
        ...
        // Calculamos las 3 l�neas de destino que corresponden a esta l�nea de origen
        ...
        for (int x = 0; x < props.sw; x++) {
            ...
            // Escribir 3 p�xeles horizontalmente en las 3 l�neas verticales
...
inline void scale4x_software(const t_scale_props& props) {
    ...
    // Nota: Aseg�rate de que tu check_center asigne a src_stride el valor de spitch/2 
    ...
    for (int y = 0; y < props.sh; y++) {
        // Puntero a la l�nea de origen actual
        uint16_t* line_src = props.src + (y * src_stride);
        
        // Calculamos las 4 l�neas de destino que corresponden a esta l�nea de origen
...
/**
* Se decide si se hace Fullscreen o Centrado P�xel Perfecto
*/
inline void finalize_scaling(const t_scale_props& props, int scaled_w, int scaled_h) {
    int t_stride = scaled_w; // Stride en p�xeles del temp_buffer
    ...
    if (!props.integer_scale) {
        ...
        // Caso B: Centrado P�xel Perfecto
        int src_s, dst_s;
...
inline void scale2x_advance(const t_scale_props& props) {
    ...
    // s_gap: Stride de la imagen original (en p�xeles uint16_t)
    ...
    // 2. Ejecuci�n del algoritmo AdvMAME2x sobre el temp_buffer
    for (int y = 0; y < props.sh; ++y) {
    ...
    // 3. LLAMADA GEN�RICA DE FINALIZACI�N
    // Aqu� se decide si se hace Fullscreen o Centrado P�xel Perfecto
...
inline void scale3x_advance(const t_scale_props& props) {
    ...
    // 1. Origen: Mantenemos tu l�gica de gaps
    ...
    for (int y = 0; y < props.sh; ++y) {
        for (int x = 0; x < props.sw; ++x) {
            ...
            if (B != H && D != F) {
                ...
                // Relleno s�lido del bloque 3x3 en temp_buffer
                for (int j = 0; j < 3; ++j) {
                    out[j * d_gap + 0] = E;
                    out[j * d_gap + 1] = E;
                    out[j * d_gap + 2] = E;
    ...
    // 3. Finalizaci�n gen�rica: decide si estirar a FS o centrar en 1280x720
    finalize_scaling(props, props.sw * 3, props.sh * 3);
}

/**
 * Scale4x para 16 bits (RGB565/555)
 * Expande cada p�xel en un bloque de 4x4 analizando sus vecinos.
 */
inline void scale4x_advance(const t_scale_props& props) {
    if (!props.src || !props.dst)
        return;

    // 1. Origen: Usamos tu l�gica de gaps
    uint16_t* s_ptr = props.src;
    const int s_gap = props.spitch / sizeof(uint16_t);

    // 2. Destino Intermedio: temp_buffer
    // El gap es exactamente el cu�druple del ancho original
    uint16_t* d_ptr = temp_buffer; 
    const int d_gap = props.sw * 4; 

    for (int y = 0; y < props.sh; ++y) {
        for (int x = 0; x < props.sw; ++x) {
            // P�xel central (E) y vecinos cardinales originales
            uint16_t E = s_ptr[y * s_gap + x];
            uint16_t B = (y > 0) ? s_ptr[(y - 1) * s_gap + x] : E;
            uint16_t D = (x > 0) ? s_ptr[y * s_gap + (x - 1)] : E;
            uint16_t F = (x < props.sw - 1) ? s_ptr[y * s_gap + (x + 1)] : E;
            ...
            if (B != H && D != F) {
                ...
                // Relleno s�lido 4x4 en temp_buffer
                for (int j = 0; j < 4; ++j) {
    ...
    // 3. Finalizaci�n: Estirar a Fullscreen o Centrado P�xel Perfecto
...
inline void convertRGB565ToARGB8888(const uint16_t* src, int sw, int sh, std::size_t spitch, uint32_t* dst, std::size_t dpitch) {
    for (int y = 0; y < sh; ++y) {
        // Calculamos el inicio de cada l�nea usando el pitch (bytes)
        ...
        for (int x = 0; x < sw; ++x) {
            ...
            // Conversi�n directa por registro (Endian-safe)
            uint32_t r = ((p >> 11) & 0x1F);
            uint32_t g = ((p >> 5) & 0x3F);
            uint32_t b = (p & 0x1F);

            // Replicaci�n de bits para color pleno (0x1F -> 0xFF)
...
/*inline void convertRGB565ToARGB8888(const uint16_t* __restrict src, int sw, int sh, std::size_t spitch, uint32_t* __restrict dst, std::size_t dpitch) {
    // Definimos el salto de l�nea en unidades del tipo de dato (no en bytes)
    ...
    for (int y = 0; y < sh; ++y) {
        ...
        for (int x = 0; x < sw; ++x) {
            ...
            uint32_t r = (p & 0xF800) << 8;  // Desplaza Rojo a su posici�n 8-bit
            uint32_t g = (p & 0x07E0) << 5;  // Desplaza Verde
            uint32_t b = (p & 0x001F) << 3;  // Desplaza Azul

            // Replicaci�n de bits r�pida (para evitar p�rdida de brillo)
 ...
 * @param spitch   Pitch (bytes por l�nea) de la superficie origen (32 bits).
 * @param dst      Puntero al destino (16 bits).
 * @param dpitch   Pitch (bytes por l�nea) de la superficie destino (16 bits).
 */
...
inline void convertARGB8888ToRGB565(const uint32_t* __restrict src, int sw, int sh, std::size_t spitch,
    ...
    if (spitch == (std::size_t)sw * 4 && dpitch == (std::size_t)sw * 2) {
        ...
        for (uint32_t i = 0; i < total_pixels; ++i) {
            uint32_t p = src[i];
            // Operaci�n combinada: reduce ciclos de reloj en el procesador Xenon
            dst[i] = ((p >> 8) & 0xF800) | ((p >> 5) & 0x07E0) | ((p >> 3) & 0x001F);
        }
    } else {
        // Fallback para cuando hay padding en las l�neas (menos com�n)
        for (int y = 0; y < sh; ++y) {
            for (int x = 0; x < sw; ++x) {
...
inline SDL_Surface* crearSuperficie16Bits(int ancho, int alto) {
    ...
    /* Configuraci�n de m�scaras seg�n el Endianness */
    #if SDL_BYTEORDER == SDL_BIG_ENDIAN
        /* Caso Xbox 360 / PowerPC: El byte m�s significativo va primero */
        ...
        /* Nota: En SDL 1.2 sobre Little Endian, a veces se requiere 
           reordenar los bytes dependiendo de c�mo se vuelquen los datos. */
    #endif
...
inline int xbrz_thread_func(void* data) {
	...
	/*xbrz::scale(
        ...
        job->w,
        job->h,                      // srcHeight
        xbrz::ColorFormat::ARGB,          // colFmt (ajustar seg�n tu xbrz.h)
        *job->cfg,                              // cfg
        job->yFirst,                                // yFirst
        job->yLast                          // yLast
    );*/
	...
	#include <ppl.h> // Librer�a nativa de VS2010 para paralelismo
#elif !defined(WANT_SDL_THREAD)
	#include <xtl.h> // Cabecera obligatoria del XDK de Xbox 360
	#include <process.h>
	
	// Funci�n worker compatible con la Xbox 360
	inline unsigned __stdcall xbrz_xbox_thread_func(void* arg) {
		...
		// La Xbox 360 tiene 3 n�cleos (0, 1, 2) con 2 hilos de hardware cada uno.
...
inline void xbrz_scale_multithread(const t_scale_props& props) {
	...
	const int numThreads = 3; // Recomendado 3 en Xbox 360 para aprovechar los 3 n�cleos Xenon
	...
	for (int i = 0; i < numThreads; i++) {
        ...
        // IMPORTANTE en Xbox 360: Asignar cada hilo a un n�cleo f�sico distinto
        // Usamos n�cleos 1, 3 y 5 para no colisionar con el hilo principal (n�cleo 0)
        XSetThreadProcessor(threads[i], (i * 2) + 1); 
    }

    // Esperar a que los 3 n�cleos terminen el procesamiento del frame
    ...
    // 4. CONVERSI�N CR�TICA: De 32 bits al temp_buffer (16 bits)
...
inline void scale_xBRZ_nx(const t_scale_props& props) {
    ...
    // Si usas el formato est�ndar de 32 bits de SDL, xbrz::RGB es suficiente.
    /*xbrz::scale(
    ...
    // 4. CONVERSI�N: De 32 bits al temp_buffer (16 bits)
    int tw = props.sw * props.scale;
    int th = props.sh * props.scale;
    ...
    // 5. LLAMADA GEN�RICA DE FINALIZACI�N
...
inline void video_blit_xbox_shader(const t_scale_props& props) {
    D3DLOCKED_RECT rect;
    // 1. Enviar datos a la textura (est� en memoria CPU_CACHED seg�n tu CreateTexture)
    ...
    // Dibujamos el Quad (los v�rtices ahora son fijos de -1 a 1, el VS hace el resto)

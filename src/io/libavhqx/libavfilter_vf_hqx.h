...
typedef int (*hqxfunc_t)(void *arg, int jobnr, int nb_jobs);
...
// 1. Cambia la estructura para que coincida con el acceso data[0] del filtro
typedef struct AVFrame {
    uint8_t *data[8];     // Array de punteros
    int linesize[8];      // Array de strides
    int width, height;
} AVFrame;
...
//static inline uint32_t rgb2yuv(const uint32_t *r2y, uint32_t c)
//{
...
{
    // Extraemos los componentes del píxel (formato 0xRRGGBB)
    int r = (c >> 16) & 0xff;
    int g = (c >> 8)  & 0xff;
    int b = c         & 0xff;

    /* Coeficientes BT.601 optimizados para enteros:
       Y = 0.299R + 0.587G + 0.114B
       U = -0.1687R - 0.3313G + 0.5B + 128
       V = 0.5R - 0.4187G - 0.0813G + 128 
    */
    int y = ( 306*r + 601*g + 117*b) >> 10;
    int u = ((-173*r - 339*g + 512*b) >> 10) + 128;
    int v = (( 512*r - 429*g -  83*b) >> 10) + 128;

    // Empaquetamos en el formato que espera hqx_filter: (Y << 16) | (U << 8) | V
    return ((uint32_t)y << 16) | ((uint32_t)(u & 0xff) << 8) | (uint32_t)(v & 0xff);
}
...
static inline void hq3x_interp_2x1(uint32_t *dst, int dst_linesize,
...
static inline void hqx_filter(const ThreadData *td, int jobnr, int nb_jobs, int n)
{
    int x, y;
...
static inline int hq##size##x(void *arg, int jobnr, int nb_jobs) \
{ \
    hqx_filter((ThreadData *)arg, jobnr, nb_jobs, size); \
    return 0; \
}
...
#include "../io/video.h"
#include <math.h> // Opcional, pero buena práctica para funciones matemáticas

// Definiciones compatibles con MSVC para tipos enteros
#define fmax(a,b) ((a) > (b) ? (a) : (b))
#define fmin(a,b) ((a) < (b) ? (a) : (b))
#define FFMAX3(a,b,c) fmax(a, fmax(b, c))
#define FFMIN3(a,b,c) fmin(a, fmin(b, c))

// El tamaño debe ser suficiente para cubrir el rango de colores indexados
// En el filtro hqx original, el array tiene 16,777,216 entradas (2^24)
//static uint32_t* tabla_rgbyuv = (uint32_t*)malloc(16777216 * sizeof(uint32_t));
//static uint32_t* tabla_rgbyuv;

/*inline int initHqxFilter(){
    uint32_t c;
    int bg, rg, g;
...
inline void hqx_software(const t_scale_props& props) {
    int n = props.scale;
    if (n < 2 || n > 4) return;

    // 1. Dimensiones del escalado hqx
    int scaled_w = props.sw * n;
    int scaled_h = props.sh * n;

    // 2. Preparar buffers temporales de 32 bits para el motor hqx
    int temp_src_pitch32 = props.sw * 4;
    int temp_dst_pitch32 = scaled_w * 4;

    uint32_t* src32 = (uint32_t*)malloc(temp_src_pitch32 * props.sh);
    uint32_t* dst32 = (uint32_t*)malloc(temp_dst_pitch32 * scaled_h);

    if (!src32 || !dst32) {
        if (src32) free(src32); if (dst32) free(dst32);
        return;
    }

    // 3. CONVERSIÓN INICIAL: 16-bit (Consola) -> 32-bit (Entrada HQX)
    convertRGB565ToARGB8888(props.src, props.sw, props.sh, props.spitch, src32, temp_src_pitch32);

    // 4. CONFIGURACIÓN AVFRAME (Usando los arrays indexados [0])
    AVFrame in_f, out_f;
    memset(&in_f, 0, sizeof(AVFrame));
    memset(&out_f, 0, sizeof(AVFrame));

    in_f.data[0]     = (uint8_t*)src32;
    in_f.linesize[0] = temp_src_pitch32; 
    in_f.width       = props.sw;
    in_f.height      = props.sh;

    out_f.data[0]     = (uint8_t*)dst32;
    out_f.linesize[0] = temp_dst_pitch32;
    out_f.width       = scaled_w;
    out_f.height      = scaled_h;

    ThreadData td;
    td.in = &in_f;
    td.out = &out_f;
    //td.rgbtoyuv = tabla_rgbyuv;

    // 5. EJECUCIÓN DEL FILTRO HQX
    hqx_filter(&td, 0, 1, n);

    // 6. CONVERSIÓN INTERMEDIA: 32-bit (Resultado HQX) -> 16-bit (temp_buffer)
    // Usamos temp_buffer como paso previo para finalize_scaling
    int t_stride_16 = scaled_w * sizeof(uint16_t);
    convertARGB8888ToRGB565(dst32, scaled_w, scaled_h, temp_dst_pitch32, temp_buffer, t_stride_16);

    // 7. FINALIZACIÓN: Centrado o Estirado FS
    // Esta función ya se encarga de mover de temp_buffer a props.dst
    finalize_scaling(props, scaled_w, scaled_h);

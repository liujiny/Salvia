...
inline void convertARGB8888ToRGB565_Fast(const uint32_t* __restrict src, int sw, int sh, std::size_t spitch,
    ...
    for (int y = 0; y < sh; ++y) {
        ...
        #ifdef _XBOX
        __dcbt(0, s); // Prefetch de la linea de origen (32 bits por p�xel consume mucho bus)
        #endif
...
inline void convert0RGB1555ToRGB565_Fast2(const uint16_t* __restrict src, unsigned width, unsigned height, std::size_t pitch, uint16_t* __restrict dst) {
    for (unsigned y = 0; y < height; y++) {
        ...
        // P�xel sobrante si el ancho es impar
        if (x < width) {
            uint32_t p = s[x];
            d[x] = ((p & 0x7FE0) << 1) | (p & 0x001F);
        }
...
// Conversi�n de un p�xel 0RGB1555 a RGB565
static inline uint16_t convert_pixel_0RGB1555_to_RGB565(uint16_t c)
{
    ...
    return ((c & 0x7C00) << 1)   // R: desplaza a posici�n 15-11
         | ((c & 0x03E0) << 1)   // G alta: desplaza a posici�n 10-6
         | ((c & 0x0200) >> 4)   // G baja: duplica MSB del verde en bit 5 (expansi�n 5 a 6 bits)
         | ((c & 0x001F));       // B: sin cambio
}

// Conversi�n de un frame completo al buffer de destino
...
{
    const unsigned src_stride = src_pitch / sizeof(uint16_t); // pitch en p�xeles

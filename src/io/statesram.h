...
void saveSram(const char* sram_path) {
    ...
    if (needsSave) {
        ...
        if (hiloBuffer) {
            ...
            // Si el hilo a�n no ha terminado el anterior, liberamos para evitar leak
...
void saveState() {
    // 1. Obtener tama�os de ambos estados
    ...
    // 2. Calcular tama�o total del buffer: Core + Marcador(4) + Tama�oRA(4) + DatosRA
    ...
    if (retro_serialize(buffer, core_state_size)) {
        ...
        // 4. Serializar datos de RetroAchievements a continuaci�n
        uint8_t* ra_ptr = (uint8_t*)buffer + core_state_size;
        
        // Escribimos un marcador "RCHV" y el tama�o para poder leerlo luego de forma segura
        ...
        if (hiloBuffer) {
            ...
            // Gesti�n de la captura de pantalla para el Slot
...
void loadState(){
    ...
    // 1. Obtener el tama�o que espera el n�cleo (Core)
    std::size_t core_state_size = retro_serialize_size();
    ...
    // 3. Cargar el estado del N�cleo (Core)
    ...
    if (bytesRead == (int)core_state_size) {
        ...
        // Intentamos leer el marcador "RCHV" y el tama�o de los datos
        ...
        LOG_ERROR("Error de lectura: El archivo es m�s peque�o de lo esperado.");
    }
        ...
        switch (actionActual) {
			case SAVE_SRAM:
				if (localBuffer) {
					...
					sd->buffer = NULL;
					sd->bufferSize = 0;
					sd->action = SAVE_NONE; // IMPORTANTE: Resetea la acci�n
					SDL_mutexV(sd->saveMutex);
				}
				break;
			case SAVE_STATE:

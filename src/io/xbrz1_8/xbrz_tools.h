...
Pix* byteAdvance(Pix* ptr, int bytes)
{
    ...
    // Forma cl·sica de avanzar punteros por bytes independientemente de la constancia:
                   ...
                   int yFirst, int yLast, PixConverter pixCvrt /*convert uint32_t to PixTrg*/)
{
    ...
    struct CoeffsX
    {
		...
		// Constructor para compatibilidad con VS2010
		CoeffsX(int _x1, int _x2, double _xx1, double _x2x) 
			: x1(_x1), x2(_x2), xx1(_xx1), x2x(_x2x) {}
		
		// Tambi√©n necesitas el constructor por defecto para std::vector::resize
		CoeffsX() : x1(0), x2(0), xx1(0), x2x(0) {} 

...
{
    static_assert(0 < M && M < N && N <= 1000, "N <= 1000");
...
double distYCbCrBuffered(uint32_t pix1, uint32_t pix2)
{
	...
	static const std::vector<float> diffToDist = []() -> std::vector<float> // <--- Tipo de retorno expl�cito
	{
		std::vector<float> tmp;
		tmp.reserve(256 * 256 * 256); // [!] Cr�tico: Evita m�ltiples realojamientos de 16MB

		for (uint32_t i = 0; i < 256 * 256 * 256; ++i)
		{
			// Usamos m�scaras de bits para extraer los bytes de forma segura
			...
			const double k_b = 0.0593; // ITU-R BT.2020
			const double k_r = 0.2627;
			...
			// square(x) debe ser (x*x). Usamos std::sqrtf para floats.
			tmp.push_back(static_cast<float>(std::sqrt(y*y + c_b*c_b + c_r*c_r)));
		}
		return tmp; 
	}(); // Los par�ntesis finales ejecutan la lambda
...
BlendResult preProcessCorners(const Kernel_4x4& ker, const xbrz::ScalerCfg& cfg) 
{
    ...
    // REEMPLAZO DE LAMBDA PARA VS2010:
    // Definimos un Functor interno que conoce ColorDistance
    struct DistWrapper {
        const xbrz::ScalerCfg& cfg_;
        DistWrapper(const xbrz::ScalerCfg& cfg) : cfg_(cfg) {}
        
        double operator()(uint32_t pix1, uint32_t pix2) const {
            // Acceso expl�cito a la clase de la plantilla
...
{
    static_assert(BLEND_NONE == 0, "BLEND_NONE must be zero");
    return b != 0;
}
...
#define i get_i<rotDeg>(ker)
    ...
    {
        // REEMPLAZO DE LAMBDAS POR MACROS PARA VS2010
        ...
        // REEMPLAZO DE doLineBlend (L�gica extra�da de la lambda)
        bool doLineBlend = true;
        if (getBottomR(blend) < BLEND_DOMINANT)
        {
            if (getTopR(blend) != BLEND_NONE && !XBRZ_EQ(e, g))
                doLineBlend = false;
            else if (getBottomL(blend) != BLEND_NONE && !XBRZ_EQ(e, c))
                doLineBlend = false;
            else if (!XBRZ_EQ(e, i) && XBRZ_EQ(g, h) && XBRZ_EQ(h, i) && XBRZ_EQ(i, f) && XBRZ_EQ(f, c))
                doLineBlend = false;
        }

        const uint32_t px = XBRZ_DIST(e, f) <= XBRZ_DIST(e, h) ? f : h;

        OutputMatrix<Scaler::scale, rotDeg> out(target, trgWidth);

        if (doLineBlend)
        {
            const double fg = XBRZ_DIST(f, g);
            const double hc = XBRZ_DIST(h, c);
...
public:
	...
	{
		const int x_p2 = x + 2; // Declaraci�n fuera del if
...
// Definimos una versi�n local para evitar conflictos con el namespace std de VS2010
template<class T>
...
void xbrz::scale(size_t factor, const uint32_t* src, uint32_t* trg, int srcWidth, int srcHeight, ColorFormat colFmt, const xbrz::ScalerCfg& cfg, int yFirst, int yLast)
{
	...
	static_assert(SCALE_FACTOR_MAX == 6, "SCALE_FACTOR_MAX must be 6");

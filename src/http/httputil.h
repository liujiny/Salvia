...
//        
//			// El peque�o Sleep que descubrimos que estabiliza el iStatus
//			Sleep(200); 
//
//			// Esperar si todav�a est� pendiente
//			if (pTest->iStatus == 0x103) { 
//				WaitForSingleObject(hDnsEvent, 5000);
//			}
//
//			if (pTest->iStatus == 0) {
//				// La IP est� en pTest->aina[0]
...
//
//		// 2. Registrar la ruta en XNet (Vital para comunicaci�n PC-Xbox)
//		IN_ADDR ipPC;
...
//
//		// 4. Intentar conectar
//		// Como es no bloqueante, devolver� SOCKET_ERROR y el error ser� WSAEWOULDBLOCK
...
//
//		// select() esperar� hasta que el socket est� listo para escribir (conectado)
//		int total = select(0, NULL, &writeSet, NULL, &tv);
//
//		if (total > 0 && FD_ISSET(s, &writeSet)) {
//			OutputDebugStringA("�CONEXI�N LOCAL EXITOSA!\n");
//		} else {
...
//		// En su lugar, vamos a forzar una peque�a espera antes del connect.
//		Sleep(1000); 
//
//		sockaddr_in target;
//		target.sin_family = AF_INET;
...
//		// Esto es m�s compatible con el stack sencillo de la 360
//		int timeout = 10000; // 3 segundos
//		setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout, sizeof(timeout));
//		setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (char*)&timeout, sizeof(timeout));
//
//		OutputDebugStringA("Intentando conexi�n directa (Bloqueante)...\n");
//    
//		// Si el connect falla aqu� con 10060, es problema de FIREWALL o RUTA
...
//		// Aunque sea una IP de internet, esto le dice a la Xbox que "conf�e" en esta ruta
//		IN_ADDR targetIP;
//		targetIP.s_addr = inet_addr("35.180.139.74"); // PortQuiz
//		
//		// 1. Notificar a la capa XNet que vamos a conectar a esta IP
//		// Esto intenta establecer la asociaci�n de seguridad (SA)
//		int result = XNetConnect(targetIP); 
//
//		if (result != 0) {
//			int err = WSAGetLastError();
//			char msg[64];
//			sprintf(msg, "XNetConnect fall� con c�digo: %d", result);
//			OutputDebugStringA(msg);
//		} else {
//			int err = WSAGetLastError();
//			char msg[64];
...
//		}
//
//		// Esta llamada es m�gica en el XDK: "Pre-autoriza" la conexi�n
//		//XNetInAddrToXnAddr(targetIP, NULL, NULL); 
//
//		SOCKET s = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
//    
//		// 3. Aumentar el buffer de env�o/recepci�n (ayuda con la estabilidad en 360)
//		int bufSize = 64 * 1024;
...
//			OutputDebugStringA("�CONEXI�N EXITOSA!\n");
//		} else {
//			int err = WSAGetLastError();
//			char msg[64];
...
//		// Para "192.168.0.1" deber�a ser 0xC0A80001

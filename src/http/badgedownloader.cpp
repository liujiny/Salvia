...
void BadgeDownloader::start() {
    if (!running) {
        ...
        if (hThread) {
			#ifdef _XBOX
			// Forzar la ejecuci�n en el n�cleo especificado
...
void BadgeDownloader::stop() {
    ...
    running = false; // Se�alizamos al hilo que debe salir del bucle while
    
    if (hThread != NULL) {
        // Esperamos un m�ximo de 2 segundos (2000 ms)
        DWORD result = WaitForSingleObject(hThread, 2000);

        if (result == WAIT_TIMEOUT) {
            // El hilo se qued� bloqueado (probablemente en un socket de red)
            // En Xbox 360, podr�as optar por TerminateThread si es cr�tico, 
            // aunque lo ideal es que tu funci�n de red tenga su propio timeout.
...
DWORD WINAPI BadgeDownloader::thread_func(LPVOID data) {
    ...
    while (self->running) {
        ...
        } // <--- EL MUTEX SE LIBERA JUSTO AQU� AUTOM�TICAMENTE

        // --- PROCESAMIENTO FUERA DEL MUTEX ---
        if (hasTask) {
            // Ahora la descarga ocurre sin bloquear a nadie m�s

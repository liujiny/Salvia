...
void CurlClient::init(){
	int err = 0;
		...
		DWORD dwStatus;
		int intentos = 0;
		const int MAX_INTENTOS = 25; // 25 * 200ms = 5 segundos m�ximo
		...
		do {
			...
			// Si da NONE de inmediato, no hay inicializaci�n o hardware b�sico
		...
		// VALIDACI�N REAL DE IP: 
		// Debe tener asignada una IP Est�tica, por DHCP o por PPPoE.
		// Si no tiene ninguno de estos bits, la IP no es v�lida para trafico.
...
extern "C" {
	...
	// 1. Soluci�n para inet_aton
    // Convierte una cadena "X.X.X.X" en una estructura de direcci�n de red (in_addr)
    int inet_aton(const char *cp, struct in_addr *inp) {
        ...
        // Validamos que ning�n byte exceda el l�mite de 255
    ...
    // 2. Soluci�n para inet_ntoa
    // Convierte una estructura in_addr en texto legible "X.X.X.X"
    char* inet_ntoa(struct in_addr in) {
        // Usamos una variable est�tica local compartiendo el comportamiento del original thread-unsafe
    ...
    // 3. Soluci�n para inet_pton
    // Equivalente seguro multiespecificaci�n, TyrQuake solo la usa para AF_INET (IPv4)
    int inet_pton(int af, const char *src, void *dst) {
        ...
        // Delegamos de forma limpia en nuestra funci�n inet_aton ya definida arriba

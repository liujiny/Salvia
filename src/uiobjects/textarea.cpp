...
bool TextArea::loadTextFile(std::string filepathToOpen) {
    // 1. Comprobaci�n r�pida para evitar recargar el mismo archivo
    ...
    while (std::getline(fileRomTxt, rawLine)) {
        ...
        while (ss >> word) {
            ...
            // Si no cabe, hacemos un salto de linea autom�tico (ajuste de texto)
            else {
...
std::vector<t_line> TextArea::wrapStringWithFont(const std::string& fulltxt, TTF_Font* font, int maxW)
{
    ...
    // Reutilizamos un �nico buffer din�mico para extraer palabras sin reservar memoria continuamente
    std::string word;
    word.reserve(32); 
    ...
    while (start < length) {
        ...
        while (end < length && fulltxt[end] != ' ') {
            if (fulltxt[end] == '\n' || fulltxt[end] == '\r') {
                hasBreak = true;
                breakPos = end;
                break; // Paramos en el salto de l�nea para procesarlo
            }
            ++end;
        }
        ...
        // 4. Evaluar si cabe en la l�nea actual
        ...
        // 5. Avanzar los �ndices seg�n lo que hemos encontrado
        if (hasBreak) {
            // Saltamos el '\n' o '\r'
            start = breakPos + 1;
            // Si el siguiente car�cter es el compa�ero (\r\n), lo saltamos tambi�n
            if (start < length && ((fulltxt[breakPos] == '\r' && fulltxt[start] == '\n') || 
                                   (fulltxt[breakPos] == '\n' && fulltxt[start] == '\r'))) {
                ++start;
            }
            // Forzamos la creaci�n de una nueva l�nea debido al salto expl�cito
            out.push_back(t_line());
            currentLineW = 0;
        } else {
            // Avanzamos al siguiente car�cter despu�s del espacio
            start = end + 1; 
        }
    }
...
std::vector<t_line> TextArea::wrapTextFileWithFont(const std::string& filepathToOpen, TTF_Font* font, int maxW)
{
    ...
    // Preasignamos un tama�o estimado inicial para evitar realojamientos del vector
    out.reserve(50); 
    out.push_back(t_line());
    ...
    // Buffer en el Stack (memoria ultrarr�pida) para leer bloques del disco
    ...
    while (file.read(buffer, BUFFER_SIZE) || file.gcount() > 0) {
        ...
        for (std::streamsize i = 0; i < bytesRead; ++i) {
            ...
            // 1. Detectar delimitadores (Espacio o Saltos de l�nea)
            if (c == ' ' || c == '\n' || c == '\r') {
                
                // Si ten�amos una palabra acumulada, la procesamos
                if (!word.empty()) {
                    ...
                    word.clear(); // Vac�a el string sin liberar su memoria interna
                }

                // 2. Gesti�n estricta de saltos de l�nea nativos (\n, \r, \r\n)
                if (c == '\n' || c == '\r') {
                    ...
                    if (c == '\r' && (i + 1 < bytesRead) && buffer[i + 1] == '\n') {
                        ...
                        file.get(); // Saltamos el \n si qued� justo en el l�mite del buffer
                    }

                    // Forzar nueva l�nea en el ajuste de texto
                    out.push_back(t_line());
                    currentLineW = 0;
                }
            } 
            else {
                // Acumular car�cter en la palabra actual
                word.push_back(c);
            }
        }
    }

    // Procesar la �ltima palabra si el fichero no terminaba en espacio/salto de l�nea
    if (!word.empty()) {
        int wordW = Fonts::getSize(font, word.c_str());
        bool hasSpace = !out.back().text.empty();
...
void TextArea::calcTicks(GameTicks gameTicks, int& scrollDesp, float& pixelDesp)
{
    ...
    if (hasScroll && scrollDesp == lastLine) {
        if (pixelDesp < PIXEL_MAX) {           // completar desplazamiento de �ltima l�nea
            advancePixels();
            return;
        }
    ...
    // --- 4. Desplazamiento suavizado por p�xeles ---
    advancePixels();
}

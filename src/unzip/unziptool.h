...
// Funci�n auxiliar para separar las extensiones y normalizarlas
...
inline int checkExtractionErrors(int result, std::size_t romsize){
	if (result < 0) {
		...
		switch (result) {
			...
			case UNZ_PARAMERROR:    errorMsg = "Error en los par�metros"; break;
			default:                errorMsg = "Error desconocido en la extracci�n"; break;
		}
		...
		LOG_DEBUG("Extracci�n completada con �xito.\n");
		return 0;
	}
}
...
inline bool isBiosFile(const std::string& filename) {
    // 1. Convertir a min�sculas una sola vez
    std::string name = filename;
    std::transform(name.begin(), name.end(), name.begin(), ::tolower);

    // 2. Filtros r�pidos (Palabras clave universales)
    if (name.find("bios") != std::string::npos) return true;
    if (name.find("boot") != std::string::npos) return true;

    // 3. Patrones espec�ficos de drivers de MAME 2003+
    static const char* patterns[] = {
        // Neo-Geo (Sistemas arcade m�s comunes)
    ...
    // 4. Bucle de b�squeda (Sencillo para el compilador de VS2010)
    ...
    // 5. Filtro por extensi�n de firmware com�n en MAME
    // Algunos firmwares no tienen nombre descriptivo pero s� estas extensiones
    if (name.size() > 4) {
...
inline int getZipFileCountFiltered(const std::string& rompath) {
    ...
    while (res == UNZ_OK) {
        ...
        std::string name = filename;
        // Convertimos a min�sculas para comparar
        std::transform(name.begin(), name.end(), name.begin(), ::tolower);
...
inline unzippedFileInfo unzipOrLoad(const std::string& rompath, const std::string& extensions, bool toMemory, const std::string& tempDir) {
    ...
    // 1. Detecci�n de ZIP por Magic Number (Endian-Safe)
    ...
    // se lee de forma distinta. Normalizamos para la comparaci�n.
    #ifdef _XBOX
    ...
    // --- CASO 2: ES UN ZIP (Extracci�n con Minizip) ---
            ...
            if (unzOpenCurrentFile(uf) == UNZ_OK) {
                ...
                if (toMemory) {
                    ...
                    if (fout) {
						...
						if (bytesRead < 0) {
							LOG_DEBUG("Error en la descompresi�n: %d\n", bytesRead);
							found = false;
						} else {
							found = true;
						}
